...
def build_ideal_condition_alert(ema_snapshot):
    ...
    return (
        ...
        f"R36F.13.2 exchange execution remains disabled."
    )
...
def synthetic_r36f12_ema_telegram_tests():
    ...
    check(
        ...
        buy['recognized'] and buy['direction'] == 'LONG'
    )

    check(
        'R36F12_TELEGRAM_SELL_COMMAND_PARSES_SHORT',
        sell['recognized'] and sell['direction'] == 'SHORT'
    )

    check(
        'R36F12_TELEGRAM_UNKNOWN_COMMAND_REJECTED',
        parse_telegram_trade_command('BUY ETH NOW')['recognized'] is False
    )
    ...
    check(
        ...
        buy_ok['authorized_preview'] is True
    )

    check(
        'R36F12_SELL_MATCHING_IDEAL_SHORT_PREVIEW_APPROVED',
        sell_ok['authorized_preview'] is True
    )

    check(
        'R36F12_DIRECTION_MISMATCH_BLOCKED',
        mismatch['authorized_preview'] is False
    )

    check(
        'R36F12_COMMAND_PREVIEW_NEVER_SENDS_ORDER',
        buy_ok.get('exchange_order_sent') is False
    )
            ...
            clusters.append(
                {
                    'minimum': min(current),
                    'maximum': max(current),
                    'average': sum(current) / Decimal(len(current)),
                    'touches': len(current)
                }
            )
            current = [value]
...
def build_cluster_diagnostics(rows, entry_price, side):
    ...
    for index, cluster in enumerate(clusters, start=1):
        log(
            ...
            f"minimum={decimal_to_string(cluster['minimum'])} "
            f"maximum={decimal_to_string(cluster['maximum'])} "
...
def calculate_tp_prices(
    ...
    if direction == 'LONG':
        tp1 = (
            entry_price
            + (cluster1 - entry_price)
            * progress1
        )

        tp2 = (
            entry_price
            + (cluster2 - entry_price)
            * progress2
        )
        ...
        tp1 = (
            entry_price
            - (entry_price - cluster1)
            * progress1
        )

        tp2 = (
            entry_price
            - (entry_price - cluster2)
            * progress2
        )
    ...
    return {
        'tp1': quantize_down(tp1, PRICE_STEP),
...
def build_cluster_tp_snapshot(
    ...
    if len(clusters) < REQUIRED_TP_CLUSTERS:
        raise RuntimeError(
            'TP approval inconsistency: diagnostics approved '
            'but independent cluster extraction found fewer '
            'than two valid clusters'
        )
    ...
    snapshot = {
        ...
        'tp1': decimal_to_string(prices['tp1']),
        'tp2': decimal_to_string(prices['tp2']),
        'tp3': {
            'type': 'TRAILING',
