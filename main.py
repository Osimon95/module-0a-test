...
async def submit_r36f15_demo_order(preview, command_preview):
    """
    R36F.15.9 second controlled WEEX demo dispatch.

    Preserves the proven R36F.15.3 ambiguous-POST reconciliation and R36F.15.4
    JIT trigger checks, adds the passed R36F.15.8 one-shot command identity,
    unique client id, pre-POST durable payload/hash evidence, restart replay
    refusal, and a GET-only current-exposure gate.
    """

    if not R36F159_DEMO_ARM_REQUESTED:
        return {"attempted": False, "sent": False, "reason": "SECOND_DEMO_ARM_NOT_REQUESTED"}
    ...
    if not R36F159_COMMAND_TOKEN:
        return {"attempted": False, "sent": False, "reason": "SECOND_DEMO_COMMAND_TOKEN_MISSING"}

    existing = read_json_file(R36F159_DEMO_JOURNAL_FILE, default={})
    command_identity = r36f159_command_identity(command_preview)

    if isinstance(existing, dict) and existing:
        existing_identity = str(existing.get("command_identity_sha256") or "").strip()

        if existing_identity and hmac.compare_digest(existing_identity, command_identity):
            reconciliation = await r36f159_reconcile_second_demo_journal(existing)
    ...
    exposure = await r36f159_reconcile_current_demo_exposure()

    if exposure.get("duplicate_entry_blocked", True):
        return {
            "attempted": False,
            "sent": False,
            "accepted": False,
            "reason": "R36F159_CURRENT_EXPOSURE_BLOCKED",
            "duplicate_block_reason": exposure.get("duplicate_block_reason"),
            "exposure": exposure,
        }

    payload = dict(preview["payload"])

    client_order_id = r36f159_client_order_id(command_preview)

    if client_order_id in set(exposure.get("existing_client_ids", [])):
        return {
            "attempted": False,
            "sent": False,
            "accepted": False,
    ...
    prepared = {
        ...
        "command": str(command_preview.get("command") or ""),
        "direction": str(command_preview.get("direction") or ""),
        "command_token_sha256": sha256_text(R36F159_COMMAND_TOKEN),
    ...
    reload_payload_hash = sha256_text(
        canonical_json(
            reloaded.get("payload", {})
        )
    )

    pre_post_reload_match = bool(
        reloaded.get("client_order_id") == client_order_id
        and
        reloaded.get("command_identity_sha256") == command_identity
        and
        reloaded.get("payload_sha256") == payload_hash
        and
        hmac.compare_digest(
            reload_payload_hash,
            payload_hash,
...
def r36f1541_classify_demo_event(
    ...
    direction = str(
        command_preview.get("direction") or "UNKNOWN"
    ).upper()
    ...
    if (
        ...
        and
        submission.get("accepted") is not True
    ):
...
def r36f1541_build_event_message(
    ...
    jit = (
        ...
        if isinstance(submission.get("jit_validation"), dict)
        else {}
    )

    journal = (
        submission.get("journal")
        if isinstance(submission.get("journal"), dict)
        else {}
    )

    transport = (
        submission.get("transport")
        if isinstance(submission.get("transport"), dict)
        else {}
    )
    ...
    lines = [
        ...
        f"Command authorized: {command_preview.get('authorized_preview', False)}",
        f"Reason: {reason}",
    ]
    ...
    if jit:
        ...
        lines.extend(
            [
                f"JIT: {'PASSED' if jit.get('valid') else 'BLOCKED'}",
...
async def send_r36f1541_state_change_alert(
    ...
    previous_event = str(
        previous.get("event_name") or ""
    )

    previous_direction = str(
        previous.get("direction") or ""
    )
    ...
    message = r36f1541_build_event_message(
        event_name,
        direction,
        command_preview,
        submission,
        ...
        if selected_direction is None:
            ...
            if not balance_readiness[
                ...
                log(
                    ...
                    + canonical_json(r36f1541_alert_result)
                )
...
async def heartbeat_loop():
    ...
    while True:
        ...
        try:
            r36f159_cycle_reconciliation = await r36f159_reconcile_current_demo_exposure()
            log(
                "R36F.15.9 CYCLE DUPLICATE BLOCKED = "
                + str(r36f159_cycle_reconciliation.get("duplicate_entry_blocked", True))
            )
            log(
                "R36F.15.9 CYCLE BLOCK REASON = "
                + str(r36f159_cycle_reconciliation.get("duplicate_block_reason"))
            )
...
async def async_main():
    ...
    try:
        startup_reconciliation = await r36f159_reconcile_current_demo_exposure()
        log(
            "R36F.15.9 STARTUP DUPLICATE BLOCKED = "
            + str(startup_reconciliation.get("duplicate_entry_blocked", True))
        )
        log(
            "R36F.15.9 STARTUP BLOCK REASON = "
            + str(startup_reconciliation.get("duplicate_block_reason"))
        )
    except Exception as exc:
        log("R36F.15.9 STARTUP RECONCILIATION ERROR = " + str(exc))
