...
PURPOSE = (
    "FROZEN EMA19/EMA50/EMA200 + TELEGRAM COMMAND INTEGRATION: preserve the "
    "proven R36F.11 TP/readiness/writer safety baseline, restore the frozen "
    "EMA crossover/confirmation/quality logic, validate BUY BTC NOW and "
    "SELL BTC NOW command parsing and direction agreement, construct ideal-"
    "condition Telegram alerts, and remain EXCHANGE ZERO-WRITE in R36F.12"
)
...
def build_ema_signal_snapshot(rows):
    ...
    crossover = detect_ema19_50_crossover(
        previous19, previous50, current19, current50
    )
    quality_ok = separation >= MIN_EMA_19_50_SEPARATION_PERCENT

    # Frozen semantics: a fresh 19/50 crossover becomes a candidate; ideal
    # direction additionally requires the full 19/50/200 stack and separation.
...
def build_ideal_condition_alert(ema_snapshot):
    ...
    return (
        f"R36F.12 IDEAL {direction} CONDITION | {SYMBOL}\n"
        ...
        "R36F.12 exchange execution remains disabled."
    )
...
async def run_r36f12():
    ...
    try:
        ...
        # R36F.12 command-flow previews prove that Telegram commands cannot bypass
        # EMA direction or TP market eligibility. No exchange request is sent.
        ...
        if current_command:
            ...
            log(
                f"R36F.12 TELEGRAM COMMAND PREVIEW = {TELEGRAM_COMMAND_PREVIEW}"
            )
    ...
    try:
        ...
        if selected_direction is None:
            ...
            if not balance_readiness[
                ...
                # R36F.12 first-live candidate is always capped at 0.0004 BTC.
                canary_quantity = min(
                    quantity,
                    CANARY_MAX_ENTRY_QUANTITY,
                )
                ...
                if configured_stop is None:
                    ...
                    diagnostic_check(
                        "R36F12_PROTECTIVE_STOP_CONFIGURED",
                        False,
                        "R36F12_CANARY_STOP_PRICE is not set; live canary remains blocked",
                    )
                    ...
                    diagnostic_check(
                        "R36F12_EXPLICIT_ARM_PRESENT",
                        protected_canary_preview[
                            "explicit_arm_requested"
                        ],
                        "R36F.12 is validation-only; leave live canary arm unset until the next controlled activation stage",
                    )

                # R36F.12 itself must never submit regardless of configuration.
                check(
                    "R36F12_LIVE_TRANSPORT_REMAINS_DISABLED",
