...
#!/usr/bin/env python3
"""
R36F.7 - EXACT WEEX V3 WRITER-CONTRACT VALIDATION CHECKPOINT
...
R36F.7 CHANGE:
    ...
    1. Preserve the R36F.7 historical two-cluster TP policy unchanged.
    2. Preserve real-market TP rejection as strategy eligibility,
       not as a bot failure.
    3. Correct the TP1/TP2 preview to the exact WEEX V3
       /capi/v3/placeTpSlOrder contract:
           planType = TAKE_PROFIT
           no unsupported side field
           executePrice = 0 for market execution after trigger
    4. Preserve the V3 entry contract:
           POST /capi/v3/order
           BUY/LONG or SELL/SHORT
           MARKET
           newClientOrderId
    5. Preserve the V3 TP3 trailing contract:
           POST /capi/v3/algoOrder
           TRAILING_MARKET
           callbackRate = configured 0.20
           workingType = MARK_PRICE
    6. Add deterministic offline schema validation for ENTRY/TP1/TP2/TP3.
    7. Add a synthetic writer-contract test independent of live market eligibility.
    8. Keep actual small-account quantity representability diagnostic-only.
    9. Keep submitted=False and ALL exchange mutation disabled.
...
PURPOSE = (
    "EXACT WEEX V3 WRITER-CONTRACT VALIDATION CHECKPOINT: "
    "preserve the proven R36F.7 strategy/safety baseline while "
    "correcting and validating ENTRY/TP1/TP2/TP3 request schemas "
...
async def run_r36f7():
    ...
    # ========================================================
    # R36F.7 SYNTHETIC EXACT-WRITER CONTRACT
    # ========================================================

    synthetic_writer_contract = None

    try:

        synthetic_writer_contract = (
            synthetic_writer_contract_test()
        )

        check(
            "EXACT_WEEX_V3_WRITER_CONTRACT",
            synthetic_writer_contract[
                "passed"
            ] is True,
        )
    ...
    # ========================================================
    # R36F.7 REAL-MARKET WRITER REQUEST CONSTRUCTION
    #
    # This remains diagnostic because market eligibility and
    # small-account quantity representability are dynamic.
    # ========================================================
    ...
    try:
        ...
        if selected_direction is None:
            ...
            # Only ENTRY/TP1/TP2/TP3 are nested validation
            # dictionaries. The remaining contract-validation
            # keys are summary booleans.
    ...
    #
    # Actual safety invariants and the deterministic exact
    # writer-contract capability remain final blockers.
    # ========================================================
    ...
    log(
        f"{STAGE} REAL_WRITER_QUANTITY_REJECTION_IS_NOT_BOT_FAILURE = TRUE"
    )
        ...
        "market_eligibility":
            {
                ...
                "tp_market_rejection_is_not_bot_failure":
                    True,

                "writer_quantity_rejection_is_not_bot_failure":
                    True,
            },
        ...
        "writer_contract_policy":
            {
                ...
                "tp1_tp2_execute_price":
                    "0",

                "tp1_tp2_side_field_present":
                    False,
