...
#!/usr/bin/env python3
"""
R36F.8 - READ-ONLY POSITION + STRICT TP QUANTITY FEASIBILITY CHECKPOINT

Purpose:
    Preserve the proven R36D/R36F.4/R36F.5.4/R36F.7 safety baseline
    while correcting ONLY two pre-live diagnostic boundaries:

        1. Restore the documented WEEX V3 single-position read route
           and sign authenticated GET query strings exactly.
        2. Reject writer construction BEFORE request building whenever
           the real entry quantity cannot represent the frozen
           20% / 20% / 60% TP quantity allocation at exchange step/minimum.

R36F.8 CHANGE:

    1. Position read endpoint:
           GET /capi/v3/account/position/singlePosition?symbol=BTCUSDT
    2. Authenticated GET signatures include the exact query string.
    3. Preserve historical two-cluster TP approval unchanged.
    4. Preserve TP price policy unchanged:
           TP1 = 20% adjustable progress toward Cluster 1
           TP2 = 50% adjustable progress toward Cluster 2
           TP3 = 60% trailing runner
    5. Preserve TP quantity allocation unchanged:
           TP1 = 20%
           TP2 = 20%
           TP3 = 60%
    6. Do NOT promote a too-small TP1/TP2 leg to exchange minimum.
    7. Do NOT redistribute quantity merely to make writer validation pass.
    8. Compute the minimum strictly representable entry quantity.
...
PURPOSE = (
    "SMALLEST READ-ONLY POSITION AND STRICT TP QUANTITY FEASIBILITY FIX: "
    "use the documented WEEX V3 single-position route, sign GET query "
    "strings exactly, and block writer construction before request building "
    "when the frozen 20/20/60 allocation is not exchange-representable"
)
...
async def weex_get(
    ...
    R36F.8 signs the exact query string for authenticated GET requests.
...
def writer_quantities(
    ...
    """
    R36F.8 strict nominal allocator.
...
# ============================================================
# R36F.8 STRICT TP QUANTITY FEASIBILITY TESTS
# ============================================================
...
async def run_r36f8():
    ...
    try:
        ...
        if selected_direction is None:
            ...
            log(
                "R36F.8 TP QUANTITY FEASIBILITY "
                + "direction="
                + selected_direction
                + " entry_qty="
                + quantity_feasibility[
                ...
                + " required_available_usdt_at_current_price="
                + decimal_to_string(
                    required_balance
                )
            )
