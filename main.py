...
def classify_exit_price(
    ...
    return (
        "MANUAL_OR_OTHER_EXIT_PLAUSIBLE"
    )
...
def run_test():
    ...
    if closing_candidates:
        ...
        chosen_exit = closing_candidates[
            0
        ]
    ...
    log(
        "R36F.15.7 TP EXECUTION VERIFIED = "
        + str(
            tp_verified
        )
    )
    ...
    log(
        "R36F.15.7 SL EXECUTION VERIFIED = "
        + str(
            sl_verified
        )
    )
    ...
    log(
        "R36F.15.7 NEW DEMO ORDER ALLOWED = "
        + str(
            new_demo_order_allowed
        )
    )
