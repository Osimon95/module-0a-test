...
async def heartbeat_loop():
    ...
    while True:
        ...
        try:
            r36f155_cycle_reconciliation = await r36f155_reconcile_existing_demo_exposure()

            log(
                "R36F.15.5 CYCLE DUPLICATE BLOCKED = "
                + str(
                    r36f155_cycle_reconciliation.get(
...
async def async_main():
    ...
    try:
        ...
        startup_reconciliation = (
            await r36f155_reconcile_existing_demo_exposure()
        )

        log(
            "R36F.15.5 STARTUP DUPLICATE BLOCKED = "
            + str(
                startup_reconciliation.get(
                    "duplicate_entry_blocked",
                    True,
                )
            )
        ...
        log(
            "R36F.15.5 STARTUP BLOCK REASON = "
            + str(
                startup_reconciliation.get(
                    "duplicate_block_reason"
                )
        ...
        log(
            "R36F.15.5 STARTUP RECONCILIATION ERROR = "
            + str(exc)
        )
