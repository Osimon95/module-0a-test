...
async def r36f_backup_demo_submission_bridge(
    preview,
    *,
    allow_post=False,
):
    """
    R36F Backup 1-3 demo submission bridge.

    Phase 1:
        allow_post=False
        ZERO-WRITE validation only.

    Phase 2:
        allow_post=True
        Submit the already-validated backup payload
        through the existing WEEX DEMO transport.

    NEVER submits a real-money order.
    """
    ...
    if not client_order_id:
        result["reason"] = (
            "BACKUP_CLIENT_ORDER_ID_MISSING"
        )
        return result
    ...
    # --------------------------------------------------------
    # REAL-MONEY FIREBREAK
    # --------------------------------------------------------

    if REAL_ORDER_EXECUTION is not False:
        result["reason"] = (
            "REAL_ORDER_FIREBREAK_NOT_INTACT"
        )
        return result

    if EXCHANGE_MUTATION_TRANSPORT_ENABLED is not False:
        result["reason"] = (
            "REAL_MUTATION_FIREBREAK_NOT_INTACT"
        )
        return result

    if ORDER_SUBMISSION_ENABLED is not False:
        result["reason"] = (
            "REAL_ORDER_SUBMISSION_FIREBREAK_NOT_INTACT"
        )
        return result
    ...
    if allow_post is not True:
        result["reason"] = (
            "BACKUP_DEMO_BRIDGE_READY_ZERO_WRITE"
        )

        log(
            "R36F BACKUP DEMO BRIDGE READY = True"
        )

        log(
            "R36F BACKUP DEMO BRIDGE BACKUP NUMBER = "
            + str(
                backup_number
            )
        )
        ...
        log(
            "R36F BACKUP DEMO BRIDGE CLIENT ID = "
            + client_order_id
        )

        log(
            "R36F BACKUP DEMO BRIDGE PAYLOAD = "
            + canonical_json(
                payload
            )
        )
        ...
        log(
            "R36F BACKUP DEMO BRIDGE WEEX POST = False"
        )

        return result

    # --------------------------------------------------------
    # PHASE 2 — WEEX DEMO POST ONLY
    # --------------------------------------------------------
    ...
    try:
        ...
        result["reason"] = (
            "BACKUP_DEMO_ACCEPTED"
            if result[
                "accepted"
            ]
            else "BACKUP_DEMO_REJECTED"
        )
