...
#!/usr/bin/env python3
"""
R36F.4 - SMALLEST TP APPROVAL GATE

Purpose:
    Preserve the proven R36D/R36F.3 safety baseline while making the
    historical TP approval flow explicit.

R36F.4 TP APPROVAL POLICY:
    ...
    IMPORTANT:
        ...
        set as a whole. TP1 is NOT independently approved with only
        one cluster.
...
PURPOSE = (
    "SMALLEST TP APPROVAL GATE: TWO VALID HISTORICAL CLUSTERS "
    "REQUIRED TO APPROVE TP1/TP2 SET"
)


# =============================================================================
# FROZEN PROVEN CONTRACT
# =============================================================================
...
def collect_ids_from_file(path):
    ...
    return collect_ids_from_json(data)
...
async def reconcile_weex():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        ticker = await weex_public_ticker(
            session
        )
    ...
    # Config values are validated from the known frozen contract.
...
def build_canary_preview():
    ...
    preview = {
        ...
        "available_balance":
            decimal_to_string(AVAILABLE_BALANCE),
        "entry_margin_percent":
            decimal_to_string(
                ENTRY_MARGIN_PERCENT
            ),
...
def evaluate_tp_approval(
    ...
    """
    R36F.4 smallest-unit change.

    The historical TP1/TP2 set is approved only when
    TWO OR MORE valid historical clusters exist.

    This is a set-level approval:
        cluster 1 -> TP1
        cluster 2 -> TP2

    One cluster is NOT enough to approve TP1 independently.
    """
    ...
    log(
        f"R36F.4_TP_APPROVAL = "
        f"{approval['status']}"
    )

    log(
        f"R36F.4_TP_APPROVAL_REASON = "
        f"{approval['reason']}"
    )

    log(
        f"R36F.4_TP_REQUIRED_CLUSTERS = "
        f"{REQUIRED_TP_CLUSTERS}"
    )

    log(
        f"R36F.4_TP_AVAILABLE_CLUSTERS = "
        f"{valid_count}"
    )
...
def build_cluster_tp_snapshot(
    ...
    log(
        f"{side} TP SET APPROVED "
        f"WITH {len(clusters)} VALID CLUSTERS"
    )
...
def synthetic_long_rows():
    ...
    for i, high in enumerate(base):
        ...
        rows.append(
            [
                i,
                str(D(high) - Decimal("500")),
                str(D(high) - Decimal("100")),
                str(D(high)),
                str(D(high) - Decimal("300")),
                "1",
            ]
        )
...
def synthetic_short_rows():
    ...
    for i, low in enumerate(base):
        ...
        rows.append(
            [
                i,
                str(D(low) + Decimal("300")),
                str(D(low) + Decimal("100")),
                str(D(low) + Decimal("500")),
                str(D(low)),
                "1",
            ]
        )
...
def synthetic_cluster_tests():
    ...
    check(
        ...
        long_snapshot["tp_approval"]["approved"]
        is True,
    )

    check(
        "SYNTHETIC_LONG_TWO_CLUSTERS",
        long_snapshot["available_valid_clusters"]
        >= 2,
    )

    long_tp1 = D(
        long_snapshot["tp1"]["price"]
    )

    long_tp2 = D(
        long_snapshot["tp2"]["price"]
    )

    check(
        "SYNTHETIC_LONG_TP_ORDERING",
        entry < long_tp1 < long_tp2,
    )
    ...
    check(
        ...
        short_snapshot["tp_approval"]["approved"]
        is True,
    )

    check(
        "SYNTHETIC_SHORT_TWO_CLUSTERS",
        short_snapshot["available_valid_clusters"]
        >= 2,
    )

    short_tp1 = D(
        short_snapshot["tp1"]["price"]
    )

    short_tp2 = D(
        short_snapshot["tp2"]["price"]
    )

    check(
        "SYNTHETIC_SHORT_TP_ORDERING",
        entry > short_tp1 > short_tp2,
    )

    # -------------------------------------------------------------
    # PRIMARY / BACKUP IMMUTABILITY CONTRACT
    # -------------------------------------------------------------
...
def synthetic_tp_rejection_test():
    ...
    check(
        ...
        approval["available_valid_clusters"] < 2,
    )
...
def build_writer_request_preview(
    ...
    """
    Constructs the exact logical writer request envelope only.
...
async def run_r36f4():
    ...
    R36A_EVIDENCE_OK = (
        OLD_R36A_UPDATE_ID in r36a_ids
    )
    ...
    # -----------------------------------------------------------------
    # REAL READ-ONLY WEEX RECONCILIATION
    # -----------------------------------------------------------------
