...
async def run_r36f12():
    ...
    try:
        ...
        diagnostic_check(
            ...
            EMA_SIGNAL_SNAPSHOT.get('ready') is True,
        ...
        if EMA_SIGNAL_SNAPSHOT.get('ready'):
            ...
            diagnostic_check(
                ...
                EMA_SIGNAL_SNAPSHOT.get('quality_ok') is True,
                f"separation={EMA_SIGNAL_SNAPSHOT.get('ema19_50_separation_percent')}%"
            )
            ...
            if ideal_alert:
                ...
                if R36F1541_ROUTINE_TELEGRAM_ALERTS_ENABLED:
                    telegram_alert_result = await send_r36f12_telegram_alert(ideal_alert)
                else:
                    telegram_alert_result = {
                        'attempted': False,
                        'sent': False,
                        'reason': 'R36F1541_ROUTINE_TELEGRAM_SUPPRESSED'
                    }

                diagnostic_check(
                    'R36F12_TELEGRAM_IDEAL_ALERT_PATH',
                    True,
                    f'enabled={R36F12_TELEGRAM_ALERTS_ENABLED} result={telegram_alert_result}'
                )
            else:
                diagnostic_check(
                    'R36F12_TELEGRAM_IDEAL_ALERT_PATH',
    ...
    if historical_rows and MARK_PRICE is not None:
        try:
            real_long_snapshot = build_cluster_tp_snapshot(
                MARK_PRICE,
                historical_rows,
                'LONG',
                'REAL_LONG_PREVIEW'
            ...
            REAL_LONG_MARKET_ELIGIBLE = bool(
                real_long_snapshot['tp_approval']['approved']
            )

            diagnostic_check(
                'REAL_LONG_TP_MARKET_ELIGIBILITY',
                REAL_LONG_MARKET_ELIGIBLE,
                'TP_APPROVAL=' + real_long_snapshot['tp_approval']['status']
            )

            log(
                'REAL_LONG_TP_APPROVAL=' +
                real_long_snapshot['tp_approval']['status']
            )
    ...
    if historical_rows and MARK_PRICE is not None:
        try:
            real_short_snapshot = build_cluster_tp_snapshot(
                MARK_PRICE,
                historical_rows,
                'SHORT',
                'REAL_SHORT_PREVIEW'
            ...
            REAL_SHORT_MARKET_ELIGIBLE = bool(
                real_short_snapshot['tp_approval']['approved']
            )

            diagnostic_check(
                'REAL_SHORT_TP_MARKET_ELIGIBILITY',
                REAL_SHORT_MARKET_ELIGIBLE,
                'TP_APPROVAL=' + real_short_snapshot['tp_approval']['status']
            )

            log(
                'REAL_SHORT_TP_APPROVAL=' +
                real_short_snapshot['tp_approval']['status']
            )
    ...
    try:
        ...
        if current_command:
            TELEGRAM_COMMAND_PREVIEW = validate_telegram_command_against_signal(
                current_command,
                EMA_SIGNAL_SNAPSHOT,
                REAL_LONG_MARKET_ELIGIBLE,
                REAL_SHORT_MARKET_ELIGIBLE
    ...
    try:
        ...
        if selected_direction is None:
            ...
            balance_readiness = evaluate_strict_tp_balance_readiness(
                AVAILABLE_BALANCE,
                MARK_PRICE,
                leverage
            )

            quantity_feasibility = evaluate_writer_quantity_feasibility(
                D(balance_readiness['planned_entry_quantity'])
            )
            ...
            log(
                ...
                + ' available_usdt=' + balance_readiness['available_balance']
                + ' planned_entry_qty=' + balance_readiness['planned_entry_quantity']
                + ' minimum_entry_qty=' + balance_readiness['minimum_strict_tp_entry_quantity']
                + ' required_margin_usdt=' + balance_readiness['required_margin_for_minimum_qty']
                + ' required_available_usdt=' + balance_readiness['required_available_balance']
                + ' shortfall_usdt=' + balance_readiness['available_balance_shortfall']
            )
            ...
            diagnostic_check(
                ...
                'reason=' + quantity_feasibility['reason']
            )
            ...
            if not balance_readiness['eligible']:
                ...
                writer_preview = build_writer_request_preview(
                    selected_direction,
                    MARK_PRICE,
                    quantity,
                    selected_snapshot
                ...
                WRITER_CONSTRUCTION_ELIGIBLE = bool(
                    writer_preview['submitted'] is False
                    and writer_preview['quantity_validation']['all_valid']
                    and writer_preview['transport_enabled'] is False
                )
                ...
                diagnostic_check(
                    ...
                    'direction=' + selected_direction
                    + ' quantity=' + writer_preview['entry_quantity']
                    + ' tp1_qty=' + writer_preview['tp1_quantity']
                    + ' tp2_qty=' + writer_preview['tp2_quantity']
                    + ' tp3_qty=' + writer_preview['tp3_quantity']
                )

                for validation_name, validation_result in writer_preview[
                    'quantity_validation'
                ].items():
                    ...
                    diagnostic_check(
                        'WRITER_QUANTITY_' + validation_name.upper(),
                        validation_result
                    )
                ...
                canary_writer_preview = build_writer_request_preview(
                    selected_direction,
                    MARK_PRICE,
                    canary_quantity,
                    selected_snapshot
                ...
                configured_stop = parse_canary_stop_price(
                ...
                r36f13_stop_price = (
                    ...
                    else calculate_r36f13_protective_stop(
                        selected_direction,
                        canary_writer_preview['entry_price']
                    )
                )

                r36f13_stop_checks = validate_r36f13_protective_stop(
                    selected_direction,
                    canary_writer_preview['entry_price'],
                    r36f13_stop_price,
                    canary_writer_preview['tp1'],
                    canary_writer_preview['tp2']
                )

                diagnostic_check(
                    'R36F13_PROTECTIVE_STOP_CALCULATED_OR_CONFIGURED',
                    r36f13_stop_checks['configured_or_calculated'],
                    'stop=' + decimal_to_string(r36f13_stop_price)
                )
                ...
                check(
                    ...
                    r36f13_stop_checks['all_valid']
                )

                r36f131_stop_envelope = validate_r36f131_stop_risk_envelope(
                    selected_direction,
                    canary_writer_preview['entry_price'],
                    r36f13_stop_price,
                    leverage
                ...
                log(
                    ...
                    + 'distance_percent='
                    + r36f131_stop_envelope['distance_percent']
                    + ' max_percent='
                    + r36f131_stop_envelope['configured_maximum_percent']
                    + ' leverage_reference_percent='
                    + r36f131_stop_envelope['leverage_reference_percent']
                )

                for envelope_check_name, envelope_check_result in (
                    r36f131_stop_envelope['checks'].items()
                ):
                    diagnostic_check(
                ...
                check(
                    ...
                    r36f131_stop_envelope['all_valid']
                )
                ...
                for loss_check_name, loss_check_result in (
                    r36f132_stop_loss_budget['checks'].items()
                ):
                ...
                for demo_check_name, demo_check_result in (
                    r36f14_demo_order_validation['checks'].items()
                ):
                ...
                protected_canary_preview = build_protected_canary_preview(
                ...
                diagnostic_check(
                    ...
                    protected_canary_preview['quantity_capped'],
                    'max=0.0004 BTC'
                )

                diagnostic_check(
                    'R36F13_DURABLE_JOURNAL_CLEAR',
                    protected_canary_preview['journal_clear']
                )
                ...
                r36f1541_alert_result = await send_r36f1541_state_change_alert(
                    TELEGRAM_COMMAND_PREVIEW,
                    r36f15_demo_submission
                )
                ...
                if r36f15_demo_submission.get('attempted'):
                    check(
                        ...
                        str(
                            r36f15_demo_submission.get('transport')
                        )
                    )
    ...
    zero_write_conditions = (
        ...
        and POSITION_MUTATION_ENABLED is False
        and FIRST_REAL_ORDER_ALLOWED is False
        and R36F14_DEMO_POST_TRANSPORT_ENABLED is False
        and R36F14_DEMO_ORDER_SUBMISSION_ENABLED is False
        and R36F14_FIRST_DEMO_ORDER_ALLOWED is False
    )
    ...
    snapshot = {
        ...
        'final_gate_ok': FINAL_GATE_OK,
        'final_blockers': FINAL_BLOCKERS,
        'weex_read_only_ok': WEEX_READ_ONLY_OK,
        'durable_evidence_ok': DURABLE_EVIDENCE_OK,
        'r36a_evidence_ok': R36A_EVIDENCE_OK,
        'r36c_evidence_ok': R36C_EVIDENCE_OK,
        'r36d_evidence_ok': R36D_EVIDENCE_OK,
        'zero_write_invariant_ok': ZERO_WRITE_INVARIANT_OK,
        'mark_price': decimal_to_string(MARK_PRICE),
        'available_balance': decimal_to_string(AVAILABLE_BALANCE),
        'open_positions': OPEN_POSITIONS,
        'long_diagnostics': LONG_DIAGNOSTICS,
        'short_diagnostics': SHORT_DIAGNOSTICS,
        'tp_policy': {
            'required_valid_clusters': REQUIRED_TP_CLUSTERS,
            'tp1_progress_percent': decimal_to_string(
                TP1_PROFIT_MARGIN_PERCENT
            ),
            'tp2_progress_percent': decimal_to_string(
                TP2_PROFIT_MARGIN_PERCENT
            ),
            'tp3_allocation_percent': decimal_to_string(
                TP3_ALLOCATION_PERCENT
            ),
            'tp1_allocation_percent': decimal_to_string(
                TP1_ALLOCATION_PERCENT
            ),
            ...
            'writer_quantity_policy': {
                'tp1_minimum': decimal_to_string(MIN_QUANTITY),
                'tp2_minimum': decimal_to_string(MIN_QUANTITY),
                'tp3_minimum': decimal_to_string(MIN_QUANTITY),
                'strict_allocation': '20/20/60',
                'minimum_strict_entry_quantity': decimal_to_string(
                    minimum_strict_tp_entry_quantity()
                ),
                'minimum_leg_promotion_allowed': False,
                'remainder_redistribution_allowed': False
            },
            'cluster_tolerance_percent': decimal_to_string(
                CLUSTER_TOLERANCE_PERCENT
            ),
            'minimum_cluster_touches': MIN_CLUSTER_TOUCHES
        },
        'synthetic_test_policy': {
            'long_requires_two_clusters': True,
            'short_requires_two_clusters': True,
            'one_cluster_must_reject': True,
            'synthetic_fixtures_changed': True,
            'production_tp_policy_changed': False
        },
        'market_eligibility': {
            'real_long_market_eligible': REAL_LONG_MARKET_ELIGIBLE,
            'real_short_market_eligible': REAL_SHORT_MARKET_ELIGIBLE
        },
        'r36f12_ema_signal': EMA_SIGNAL_SNAPSHOT,
        'r36f12_telegram_command_preview': TELEGRAM_COMMAND_PREVIEW,
        'r36f12_telegram_policy': {
            'buy_command': TELEGRAM_BUY_COMMAND,
            'sell_command': TELEGRAM_SELL_COMMAND,
            'alerts_enabled': R36F12_TELEGRAM_ALERTS_ENABLED,
            'command_can_execute_exchange_order': False,
            'requires_ema_direction_agreement': True,
            'requires_tp_market_eligibility': True
        },
        'canary_preview': canary_preview,
        'writer_preview': writer_preview,
        'r36f12_protected_canary_preview': protected_canary_preview,
        'r36f13_protective_stop': {
            'distance_percent': decimal_to_string(
                R36F13_PROTECTIVE_STOP_DISTANCE_PERCENT
            ),
            'price': (
                decimal_to_string(r36f13_stop_price)
                if r36f13_stop_price is not None
                else None
            ),
            'checks': r36f13_stop_checks,
            'mandatory_before_authorization': True,
            'exchange_order_sent': False
        },
        'r36f131_stop_risk_envelope': {
            'configured_maximum_percent': decimal_to_string(
                R36F131_MAX_PROTECTIVE_STOP_DISTANCE_PERCENT
            ),
            'evaluation': r36f131_stop_envelope,
            'mandatory_before_authorization': True,
            'leverage_reference_is_not_liquidation_price': True,
            'exchange_order_sent': False
        },
        'r36f132_stop_loss_budget': {
            'configured_max_account_loss_percent': decimal_to_string(
                R36F132_MAX_ACCOUNT_LOSS_PERCENT
            ),
        ...
        'r36f12_canary_safety': {
            'max_entry_quantity': decimal_to_string(
                CANARY_MAX_ENTRY_QUANTITY
            ),
            'stop_price_configured': bool(
                CANARY_STOP_PRICE_TEXT
            ),
            'explicit_arm_requested': CANARY_ARM_REQUESTED,
            'durable_journal_file': R36F12_CANARY_JOURNAL_FILE,
            'durable_journal_unresolved': unresolved_canary_journal(
                read_json_file(
                    R36F12_CANARY_JOURNAL_FILE,
                    default={}
                )
            ),
            'r36f12_zero_write': True
        },
        'strict_tp_quantity_feasibility': quantity_feasibility,
        'strict_tp_balance_readiness': balance_readiness,
        'trade_readiness': {
            'status': (
                balance_readiness.get('status')
                if balance_readiness
                else 'UNAVAILABLE'
            ),
            'reason': (
                balance_readiness.get('reason')
                if balance_readiness
        ...
        'writer_construction_eligible': WRITER_CONSTRUCTION_ELIGIBLE,
        'execution_firebreak': {
            'real_order_execution': REAL_ORDER_EXECUTION,
            'demo_order_execution': DEMO_ORDER_EXECUTION,
            'exchange_mutation_transport_enabled': EXCHANGE_MUTATION_TRANSPORT_ENABLED,
            ...
            'margin_mode_mutation_enabled': MARGIN_MODE_MUTATION_ENABLED,
            ...
            'r36f15_demo_post_transport_enabled': R36F15_DEMO_POST_TRANSPORT_ENABLED,
            'r36f15_demo_order_submission_enabled': R36F15_DEMO_ORDER_SUBMISSION_ENABLED,
            'r36f15_demo_arm_requested': R36F15_DEMO_ARM_REQUESTED
        },
        ...
        'writer_endpoints': {
            'entry': WRITER_ENDPOINT_ENTRY,
            'tp1_tp2': WRITER_ENDPOINT_TPSL,
            'tp3': WRITER_ENDPOINT_TRAILING
        },
        'writer_submission_policy': {
            'submitted': False,
            'post_requests_sent': False,
            'exchange_mutation_sent': False
        }
    }
    ...
    if (
        ...
        log(
            'WEEX DEMO ORDER WAS SENT; '
            'PRODUCTION REAL-MONEY ORDER REMAINS DISABLED'
        )
    else:
        log(
...
async def heartbeat_loop():
    ...
    while True:
        ...
        log(
            f"HEARTBEAT stage={STAGE} "
            f"status={TEST_STATUS} "
            f"count={HEARTBEAT_COUNT} "
            f"r36a_id={OLD_R36A_UPDATE_ID} "
            f"tp1_margin={decimal_to_string(TP1_PROFIT_MARGIN_PERCENT)} "
            f"tp2_margin={decimal_to_string(TP2_PROFIT_MARGIN_PERCENT)} "
            f"required_clusters={REQUIRED_TP_CLUSTERS} "
            f"long_valid_clusters={LONG_DIAGNOSTICS.get('valid_cluster_count')} "
            f"short_valid_clusters={SHORT_DIAGNOSTICS.get('valid_cluster_count')} "
            f"write_transport={EXCHANGE_MUTATION_TRANSPORT_ENABLED} "
            f"real_execution={REAL_ORDER_EXECUTION} "
            f"demo_arm={R36F15_DEMO_ARM_REQUESTED} "
            f"reevaluation_seconds={R36F151_REEVALUATION_SECONDS}"
        )
        ...
        try:
            ...
            log(
                f'{STAGE} RUNTIME REEVALUATION ERROR = '
                f'{exc}'
            )
