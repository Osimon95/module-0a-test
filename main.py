#!/usr/bin/env python3
"""
R36F.1 - SMALLEST TP ENGINE VALIDATION FIX

Purpose:
    Preserve the R36F/R36D/R36E safety baseline while making ONLY
    the smallest validation improvement:

        1. Preserve real historical LONG TP preview.
        2. Add real historical SHORT TP preview.
        3. Require both real LONG and SHORT TP previews to pass.
        4. Preserve all existing safety, reconciliation, durable
           evidence, synthetic TP, quantity preview, and zero-write
           behavior.
...
PURPOSE = (
    "R36F.1 - SMALLEST TP ENGINE VALIDATION FIX: "
    "HISTORICAL LONG + SHORT CLUSTER TP PREVIEWS"
)
...
def run_r36f():
    ...
    log(
        "R36F.1 TEST 6: "
        "CLUSTER TP ENGINE + REAL LONG/SHORT HISTORICAL PREVIEWS"
    )
    ...
    # ------------------------------------------------------------------
    # REAL HISTORICAL TP PREVIEWS
    #
    # R36F already tested LONG.
    # R36F.1 adds SHORT using the exact same TP engine.
    #
    # Both are READ ONLY.
    # ------------------------------------------------------------------

    real_long_tp_ok = False
    real_short_tp_ok = False

    real_long_tp_snapshot = None
    real_short_tp_snapshot = None
    ...
    if (
        ...
        # ------------------------------------------------------------------
        # REAL SHORT PREVIEW - R36F.1 ONLY ADDITION
        # ------------------------------------------------------------------

        try:

            real_short_tp_snapshot = (
                calculate_cluster_tp_snapshot(
                    CURRENT_MARK_PRICE,
                    "SHORT",
                    k_rows,
    ...
    check(
        "R36F.1 Performed Zero Exchange Writes",
        zero_write_ok,
    )
    ...
    snapshot = {
        ...
        "r36f_tp_engine": {
            ...
            "real_primary_tp_preview_long":
                real_long_tp_snapshot,

            "real_primary_tp_preview_short":
                real_short_tp_snapshot,

            "real_long_tp_preview_ok":
                real_long_tp_ok,

            "real_short_tp_preview_ok":
                real_short_tp_ok,

            "real_long_short_tp_validation_ok":
                real_tp_ok,
        },
    ...
    # Snapshot write.
    #
    # We perform the write only after the gate has been evaluated.
    # If it fails, rebuild the audit object so the failure is reflected
    # consistently in the persisted object when possible.
    # ------------------------------------------------------------------
    ...
    # If the first write failed, the object itself could not have
    # been persisted. We nevertheless update the in-memory snapshot
    # so logs and final-state logic remain internally consistent.
    ...
    if not snapshot_written:
        ...
        snapshot["test_status"] = TEST_STATUS
        snapshot["r36f_pre_live_gate"] = "FAIL"
        snapshot["blockers"] = FINAL_BLOCKERS
    ...
    check(
        "R36F.1 Final Pre-Live Production Gate",
        final_gate,
    )
    ...
    if final_gate:
        ...
        log(
            "R36F.1 RESULT: "
            "LONG + SHORT TP VALIDATION PASSED"
        )

        log(
            "NEXT_STAGE=R36G_FIRST_LIVE_CANARY"
        )
