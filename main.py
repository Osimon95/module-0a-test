...
PURPOSE = (
    "R36F.15.10.4b LIVE CLASSIFIER + CONFIRMED MODE CONTROLLER MERGER: "
    ...
    "Production real-money execution and demo execution remain hard-disabled."
)
...
#
# It upgrades the existing R36F15103 controller used by the
# frozen R36F.15.10.3 runtime.
#
# Existing runtime variables remain authoritative:
#
...
#
# Therefore heartbeat_loop(), run_r36f12() and the existing
# 60-second reevaluation continue reading the SAME state.
#
# No parallel classifier.
# No parallel reevaluation loop.
# No second transition state.
#
# ============================================================
...
def r36f15103_raw_classifier(
    ...
    # BREAKOUT
    #
    # Strong EMA direction plus sufficiently large
    # short-term movement.
    #
    if (
        strong_direction
    ...
    # STRUCTURE
    #
    # Strong EMA direction and at least two usable
    # historical clusters.
    #
    if (
        strong_direction
    ...
    # BREAKOUT FALLBACK
    #
    # Strong directional market but historical two-cluster
    ...
    # SCALP
    #
    # Normal smaller-movement environment without confirmed
    # STRUCTURE or BREAKOUT conditions.
    #
    return (
        "SCALP",
...
# ============================================================
# R36F.15.10.4b CONFIRMED TRANSITION CONTROLLER
# ============================================================
...
def r36f15103_update_mode(
    ...
    #
    # Once exposure exists, classification can continue,
    # but the mode governing that trade cannot change.
    #
    if (
        trade_active
        and
    ...
    #
    # RAW MODE AGREES WITH ACTIVE MODE.
    #
    # Any incomplete transition is cancelled.
    #
    if (
        raw_mode
        == R36F15103_ACTIVE_MODE
...
# ============================================================
# EXISTING 10.3 MERGE CYCLE — 4b CONTROLLER INTEGRATED
# ============================================================
...
def r36f15103_merge_cycle(
    ...
    # IMPORTANT:
    #
    # This is the ONLY transition-controller call.
    #
    active_mode = (
    ...
    #
    # R36F.15.10.4b EXCLUSIVE MODE INVARIANT
    #
    if (
        active_mode
        not in R36F15103_VALID_MODES
    ...
    if (
        ...
        raise RuntimeError(
            "R36F.15.10.4b MULTIPLE ACTIVE MODES"
        )
    ...
    result = {
        ...
        "mode_change_confirmations_required":
            R36F15103_MODE_CONFIRMATIONS_REQUIRED,
    ...
    log(
        ...
        f"exclusive_ok={result['exclusive_mode_ok']} "
...
# ============================================================
# R36F.15.10.4b STARTUP DIAGNOSTIC
# ============================================================
...
def r36f15103_startup_diagnostic():
    ...
    log(
        ...
        "TRANSITION CONTROLLER MERGER LOADED"
    )

    log(
        "R36F.15.10.4b "
        "MODES=SCALP|STRUCTURE|BREAKOUT"
    )
