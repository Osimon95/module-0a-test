...
#   SYNTHETIC DISPATCH
#       ↓
#   COMPLETE SYNTHETIC POSITION LIFECYCLE
#       ↓
#   TP1 / TP2 / TP3
#       ↓
#   TRAILING PROTECTION
...
def parse_filter_number(record, filter_types, field_names):
    ...
    wanted = {str(x).upper() for x in filter_types}
...
def obtain_contract_information():
    ...
    if quantity_precision is None:
        quantity_precision = contract.get("quantityScale")

    if quantity_precision is None:
        quantity_precision = contract.get("volumePlace")

    price_precision = contract.get("pricePrecision")

    if price_precision is None:
        price_precision = contract.get("priceScale")

    if price_precision is None:
        price_precision = contract.get("pricePlace")
    ...
    if price_step is None or price_step <= 0:
        ...
        if price_precision is not None:
            ...
            raise RuntimeError(
                "Unable to parse contract price step"
            )
...
class SyntheticPositionLifecycle:
    ...
    def pyramid_eligible(self, price):
        ...
        return gain >= TP1_TRIGGER_PERCENT
    ...
    def snapshot(self):
        ...
        return {
            ...
            "networkWriteAllowed":
                self.network_write_allowed,
            "state": self.state,
            "entryPrice":
                decimal_text(self.entry_price),
            "initialQuantity":
                decimal_text(self.initial_quantity),
            "remainingQuantity":
                decimal_text(self.remaining_quantity),
            "closedQuantity":
                decimal_text(self.closed_quantity),
            "highestPrice":
                decimal_text(self.highest_price),
...
def run_validation():
    ...
    check(
        ...
        ALLOW_ACCOUNT_MUTATION is False,
    )
    ...
    balance_path, available_usdt = (
        obtain_available_usdt()
    )
    ...
    check(
        "BTCUSDT Position Records Were Parsed",
        isinstance(symbol_positions, list),
    )

    check(
        "BTCUSDT Open Position Records Were Parsed",
        isinstance(open_positions, list),
    )
    ...
    print(
        f"{VERSION}: ISOLATED LONG="
        f"{decimal_text(long_leverage)}x",
        flush=True,
    )

    print(
        f"{VERSION}: ISOLATED SHORT="
    ...
    market_price_path, market_price = (
        obtain_mark_price()
    )
    ...
    print(
        f"{VERSION}: PLANNED INITIAL NOTIONAL="
    ...
    check(
        ...
        decision.get("synthetic") is True,
    )

    check(
        "Decision Action Exists",
        bool(decision.get("action")),
    )
    ...
    print(
        ...
        f"{decimal_text(normalized_quantity)} BTC",
        flush=True,
    )

    print(
        f"{VERSION}: NORMALIZED NOTIONAL="
        f"{decimal_text(normalized_notional)} USDT",
        flush=True,
    )

    print(
        f"{VERSION}: NORMALIZED MARGIN AT 100x="
    ...
    live_state = {
        ...
        "positionMode":
            position_mode,
        "isolatedLongLeverage":
            decimal_text(long_leverage),
        "isolatedShortLeverage":
            decimal_text(short_leverage),
        "targetMarginType":
            TARGET_MARGIN_TYPE,
        "targetLeverage":
            decimal_text(TARGET_LEVERAGE),
        "initialEntryPercent":
            decimal_text(
                INITIAL_ENTRY_PERCENT
            ),
    ...
    check(
        ...
        bool(live_state_hash),
    )

    print(
        f"{VERSION}: LIVE STATE="
        f"{canonical_json(live_state)}",
        flush=True,
    )
    ...
    check(
        ...
        synthetic_intent["symbol"]
        == SYMBOL,
    )

    check(
        "Intent Side Is BUY",
        synthetic_intent["side"]
        == "BUY",
    )
    ...
    check(
        ...
        synthetic_intent["type"]
        == "MARKET",
    )

    check(
        "Intent Quantity Matches Normalized Quantity",
        D(synthetic_intent["quantity"])
        == normalized_quantity,
    )

    print(
        f"{VERSION}: SYNTHETIC INTENT="
        f"{canonical_json(synthetic_intent)}",
        flush=True,
    )

    print(
        f"{VERSION}: SYNTHETIC INTENT SHA256="
        f"{synthetic_intent_hash}",
        flush=True,
    )
    ...
    check(
        ...
        synthetic_payload["symbol"]
        == synthetic_intent["symbol"],
    )

    check(
        "Payload Side Matches Intent",
        synthetic_payload["side"]
        == synthetic_intent["side"],
    )
    ...
    check(
        "Payload Hash Exists",
        bool(synthetic_payload_hash),
    )

    print(
        f"{VERSION}: SYNTHETIC PAYLOAD="
        f"{canonical_json(synthetic_payload)}",
        flush=True,
    )

    print(
        f"{VERSION}: SYNTHETIC PAYLOAD SHA256="
        f"{synthetic_payload_hash}",
        flush=True,
    )
    ...
    check(
        "Position Lifecycle Is Synthetic Only",
        lifecycle.synthetic_only,
    )

    check(
        "Lifecycle Forbids Network Writes",
        lifecycle.network_write_allowed
        is False,
    )

    check(
        "Synthetic Position Starts OPEN",
        lifecycle.state == "OPEN",
    )

    check(
        "Synthetic Entry Price Matches Live Reference Price",
        lifecycle.entry_price
        == market_price,
    )

    check(
        "Synthetic Quantity Matches Entry Quantity",
        lifecycle.initial_quantity
        == normalized_quantity,
    )
    ...
    test_header(
        20,
        "TP1 LIFECYCLE TRANSITION",
    )

    tp1_price = (
        lifecycle.price_for_percent(
            TP1_TRIGGER_PERCENT
        )
    )

    lifecycle.process_profit_price(
        tp1_price
    )
    ...
    check(
        "TP2 Has Not Triggered Yet",
        lifecycle.tp2_done is False,
    )
    ...
    check(
        "TP1 Reduced Remaining Quantity",
        lifecycle.remaining_quantity
        < lifecycle.initial_quantity,
    )

    print(
        f"{VERSION}: TP1 TEST PRICE="
        f"{decimal_text(tp1_price)}",
        flush=True,
    )
    ...
    check(
        "Trailing Protection Armed",
        lifecycle.trailing_armed,
    )

    check(
        "Position Remains Open After TP2",
        lifecycle.state == "OPEN",
    )

    print(
        f"{VERSION}: TP2 TEST PRICE="
        f"{decimal_text(tp2_price)}",
        flush=True,
    )
    ...
    lifecycle.process_trailing_price(
        trailing_high_price
    )

    check(
        "Trailing High Increased",
        lifecycle.highest_price
        >= trailing_high_price,
    )

    check(
        "Trailing Exit Has Not Triggered At New High",
        lifecycle.trailing_triggered
        is False,
    )

    check(
        "Position Remains Open At New High",
        lifecycle.state == "OPEN",
    )

    print(
        f"{VERSION}: TRAILING HIGH="
        f"{decimal_text(lifecycle.highest_price)}",
        flush=True,
    )
    ...
    check(
        "Synthetic Position Is Closed",
        lifecycle.state == "CLOSED",
    )

    check(
        "Synthetic Remaining Quantity Is Zero",
        lifecycle.remaining_quantity
        == Decimal("0"),
    )

    print(
        f"{VERSION}: TRAILING EXIT PRICE="
        f"{decimal_text(trailing_stop)}",
        flush=True,
    )
    ...
    pyramid_price = (
        pyramid_test.price_for_percent(
            TP1_TRIGGER_PERCENT
        )
    )

    check(
        "First Pyramid Is Eligible",
        pyramid_test.pyramid_eligible(
            pyramid_price
        ),
    )

    check(
        "First Synthetic Pyramid Candidate Accepted",
        pyramid_test.synthetic_pyramid(
            pyramid_price
        ),
    )
    ...
    check(
        ...
        pyramid_test.pyramid_adds == 1,
    )

    check(
        "Second Pyramid Is Rejected",
        pyramid_test.synthetic_pyramid(
            pyramid_price
        ) is False,
    )
    ...
    backup_price_1 = (
        backup_test.adverse_price_for_percent(
            Decimal("0.30")
        )
    )
    ...
    check(
        "Backup One Is Eligible",
        backup_test.synthetic_backup(
            backup_price_1
        ),
    )

    check(
        "Backup Two Is Eligible",
        backup_test.synthetic_backup(
            backup_price_2
        ),
    )
    ...
    duplicate_first_allowed = (
        first_client_id
        not in active_client_ids
    )

    if duplicate_first_allowed:
        active_client_ids.add(
            first_client_id
        )
    ...
    check(
        "Initial Client Order ID Was Unique",
        duplicate_first_allowed,
    )

    check(
        "Client Order ID Was Registered",
        first_client_id
    ...
    check(
        "Anti Duplicate Toggle Remains Enabled",
        ANTI_DUPLICATE_ORDERS,
    )
    ...
    check(
        "Synthetic Long Position Is Active",
        synthetic_long_active,
    )

    check(
        "Opposite Direction Candidate Exists",
        synthetic_short_candidate,
    )

    check(
        "Opposite Direction Entry Is Rejected",
        short_allowed is False,
    )
    ...
    reversal_price = (
        reversal_test.adverse_price_for_percent(
            Decimal("0.20")
        )
    )

    reversal_result = (
        reversal_test.trend_reversal_exit(
            reversal_price
        )
    )
    ...
    check(
        "Synthetic Reversal Exit Was Accepted",
        reversal_result,
    )

    check(
        "Reversal Exit Closed Synthetic Position",
        reversal_test.state
        == "CLOSED",
    )

    check(
        "Reversal Exit Reason Was Recorded",
        reversal_test.exit_reason
        == "TREND_REVERSAL_EXIT",
    )
    ...
    loss_price = (
        cooldown_test.adverse_price_for_percent(
            Decimal("0.50")
        )
    )
    ...
    check(
        "Synthetic Loss Exit Was Accepted",
        loss_result,
    )

    check(
        "Loss Exit Closed Synthetic Position",
        cooldown_test.state
        == "CLOSED",
    )

    check(
        "Cooldown Starts After Loss Exit",
    ...
    remaining_before = (
        terminal_test.remaining_quantity
    )
    ...
    check(
        "Terminal Quantity Remains Zero",
        terminal_test.remaining_quantity
        == remaining_before
        == Decimal("0"),
    )
    ...
    check(
        ...
        terminal_test.pyramid_adds
        == 0,
    )

    check(
        "Terminal Position Cannot Backup",
        terminal_test.backups
        == 0,
    )
    ...
    test_header(
        32,
        "LIFECYCLE EVENT JOURNAL BINDING",
    )
    ...
    lifecycle_hash = sha256_json(
        lifecycle_snapshot
    )

    check(
        "Lifecycle Snapshot Exists",
        bool(lifecycle_snapshot),
    )

    check(
        "Lifecycle Event Journal Exists",
        len(lifecycle.events) > 0,
    )

    check(
        "Lifecycle Snapshot Is Synthetic",
        lifecycle_snapshot[
            "syntheticOnly"
        ] is True,
    )
    ...
    check(
        "Lifecycle Snapshot Forbids Network Write",
        lifecycle_snapshot[
            "networkWriteAllowed"
        ] is False,
    )
    ...
    check(
        "Lifecycle Hash Exists",
        bool(lifecycle_hash),
    )

    print(
        f"{VERSION}: LIFECYCLE="
        f"{canonical_json(lifecycle_snapshot)}",
        flush=True,
    )

    print(
        f"{VERSION}: LIFECYCLE SHA256="
        f"{lifecycle_hash}",
        flush=True,
    )
    ...
    test_header(
        33,
        "COMPLETE STRATEGY EXECUTION BINDING",
    )
    ...
    check(
        "Complete Binding Forbids Transmission",
        complete_binding[
            "transmissionAllowed"
        ] is False,
    )
    ...
    check(
        "Complete Binding Forbids Network Write",
        complete_binding[
            "networkWriteAllowed"
        ] is False,
    )
    ...
    check(
        "Complete Binding Preserves Live State",
        complete_binding[
            "liveStateSHA256"
        ]
        == live_state_hash,
    ...
    check(
        "Complete Binding Preserves Payload",
        complete_binding[
            "payloadSHA256"
        ]
        == synthetic_payload_hash,
    ...
    check(
        "Complete Binding Preserves Lifecycle",
        complete_binding[
            "lifecycleSHA256"
        ]
        == lifecycle_hash,
    ...
    check(
        "Complete Binding Hash Exists",
        bool(complete_binding_hash),
    )

    print(
        f"{VERSION}: COMPLETE BINDING="
        f"{canonical_json(complete_binding)}",
        flush=True,
    )

    print(
        f"{VERSION}: COMPLETE BINDING SHA256="
        f"{complete_binding_hash}",
        flush=True,
    )
    ...
    runtime_phase = (
        "FULL_SYNTHETIC_LIFECYCLE_VALIDATED"
    )
    ...
    print(
        f"{VERSION}: LIFECYCLE EVENTS="
        f"{len(lifecycle.events)}",
        flush=True,
    )
    ...
    print(
        f"{VERSION}: COMPLETE SYNTHETIC "
        f"STRATEGY LIFECYCLE VALIDATED",
        flush=True,
    )
