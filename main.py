...
def allocation_exactly_representable(entry_quantity, tp1_percent, tp2_percent, tp3_percent):
    ...
    quantities = [
        entry_quantity * percent / Decimal("100")
        for percent in percentages
    ]

    return bool(
        entry_quantity >= MIN_QUANTITY
        and all(q >= MIN_QUANTITY for q in quantities)
        and all(quantize_down(q, QUANTITY_STEP) == q for q in quantities)
...
def validate_writer_quantities(entry_quantity, tp1, tp2, tp3):
    ...
    checks = {
        ...
        "entry_on_step": quantize_down(entry_quantity, QUANTITY_STEP) == entry_quantity,
        "tp1_on_step": quantize_down(tp1, QUANTITY_STEP) == tp1,
        "tp2_on_step": quantize_down(tp2, QUANTITY_STEP) == tp2,
        "tp3_on_step": quantize_down(tp3, QUANTITY_STEP) == tp3,
        "entry_minimum": entry_quantity >= MIN_QUANTITY,
        "tp1_minimum": tp1 >= MIN_QUANTITY,
        "tp2_minimum": tp2 >= MIN_QUANTITY,
        "tp3_minimum": tp3 >= MIN_QUANTITY,
        "allocation_sum_exact": (tp1 + tp2 + tp3) == entry_quantity,
        "tp1_selected_percent_exact": tp1 == exact_tp1,
        "tp2_selected_percent_exact": tp2 == exact_tp2,
        "tp3_selected_percent_exact": tp3 == exact_tp3,
        "tp3_non_negative": tp3 >= Decimal("0"),
    }
...
def evaluate_writer_quantity_feasibility(entry_quantity):
    ...
    return {
        ...
        "reason": (
            ...
            if feasible
            else "POSITION_TOO_SMALL_OR_NOT_REPRESENTABLE_BY_APPROVED_TP_ALLOCATIONS"
        ),
        ...
        "requested_allocation": "20/20/60",
        "selected_allocation": allocation["label"] if allocation else None,
        "allocation_adjusted": bool(allocation and allocation["adjusted"]),
        "selected_tp1_percent": decimal_to_string(allocation["tp1_percent"]) if allocation else None,
        "selected_tp2_percent": decimal_to_string(allocation["tp2_percent"]) if allocation else None,
        "selected_tp3_percent": decimal_to_string(allocation["tp3_percent"]) if allocation else None,
        "minimum_required_entry_quantity": decimal_to_string(minimum_required),
        "checks": checks,
    }
...
def evaluate_strict_tp_balance_readiness(available_balance, mark_price, leverage):
    ...
    return {
        "eligible": eligible,
        "status": "ELIGIBLE" if eligible else "TRADE_NOT_ELIGIBLE",
        "reason": (
            "ADJUSTABLE_TP_BALANCE_AND_QUANTITY_READY"
            if eligible
            else "INSUFFICIENT_BALANCE_FOR_APPROVED_TP_ALLOCATION"
        ),
        ...
        "minimum_strict_tp_entry_quantity": decimal_to_string(minimum_entry_quantity),
        "required_margin_for_minimum_qty": decimal_to_string(required_entry_margin),
        "required_available_balance": decimal_to_string(required_available_balance),
        ...
        "requested_allocation": quantity_feasibility["requested_allocation"],
        "selected_allocation": quantity_feasibility["selected_allocation"],
        "allocation_adjusted": quantity_feasibility["allocation_adjusted"],
...
def build_writer_request_preview(
    ...
    return {
        ...
        "allocation_percent": {
            "tp1": decimal_to_string(select_tp_allocation(entry_quantity)["tp1_percent"]),
            "tp2": decimal_to_string(select_tp_allocation(entry_quantity)["tp2_percent"]),
            "tp3": decimal_to_string(select_tp_allocation(entry_quantity)["tp3_percent"]),
        },

        "allocation_label": select_tp_allocation(entry_quantity)["label"],

        "allocation_adjusted": select_tp_allocation(entry_quantity)["adjusted"],
...
def validate_weex_v3_writer_shapes(writer_preview):
    ...
    entry_required = {
        "endpoint", "method", "symbol", "side", "positionSide", "type",
        "quantity", "newClientOrderId", "reduceOnly",
    }
    tpsl_required = {
        "endpoint", "method", "symbol", "positionSide", "planType",
        "triggerPrice", "executePrice", "quantity", "triggerPriceType",
        "clientAlgoId", "reduceOnly",
    }
    trailing_required = {
        "endpoint", "method", "symbol", "side", "positionSide", "type",
        "quantity", "callbackRate", "workingType", "clientAlgoId", "reduceOnly",
    }

    checks = {
        "entry_endpoint": entry.get("endpoint") == "/capi/v3/order",
        "entry_method": entry.get("method") == "POST",
        "entry_required_fields": entry_required.issubset(entry.keys()),
        "entry_market_type": entry.get("type") == "MARKET",
        "entry_reduce_only_false": entry.get("reduceOnly") is False,
        "tp1_endpoint": tp1.get("endpoint") == "/capi/v3/placeTpSlOrder",
        "tp2_endpoint": tp2.get("endpoint") == "/capi/v3/placeTpSlOrder",
        "tp1_plan_type": tp1.get("planType") == "TAKE_PROFIT",
        "tp2_plan_type": tp2.get("planType") == "TAKE_PROFIT",
        "tp1_required_fields": tpsl_required.issubset(tp1.keys()),
        "tp2_required_fields": tpsl_required.issubset(tp2.keys()),
        "tp1_no_legacy_side": "side" not in tp1,
        "tp2_no_legacy_side": "side" not in tp2,
        "tp1_no_legacy_type": "type" not in tp1,
        "tp2_no_legacy_type": "type" not in tp2,
        "trailing_endpoint": tp3.get("endpoint") == "/capi/v3/algoOrder",
        "trailing_type": tp3.get("type") == "TRAILING_MARKET",
        "trailing_required_fields": trailing_required.issubset(tp3.keys()),
    }
    checks["all_valid"] = all(checks.values())
    return checks
...
def calculate_r36f13_protective_stop(direction, entry_price):
    """Calculate the mandatory preview protective stop; never submits it."""
    entry_price = D(entry_price)
    ...
    if direction == "LONG":
        ...
        raw_stop = entry_price * (Decimal("1") + distance)
        # ROUND_DOWN is safe for step normalization, but SHORT stops must remain
        # strictly above entry; lift one step if normalization collapses them.
        stop_price = quantize_down(raw_stop, PRICE_STEP)
...
def validate_r36f13_protective_stop(direction, entry_price, stop_price, tp1_price, tp2_price):
    ...
    checks = {
        "configured_or_calculated": True,
        "positive": positive,
        "correct_side_of_entry": correct_side,
        "price_step_normalized": on_step,
        "does_not_cross_entry_or_tp": separated_from_tp,
    }
    checks["all_valid"] = all(checks.values())
    return checks
...
def unresolved_canary_journal(journal):
    if not journal:
        return False
    return str(journal.get("status", "")).upper() in {
        "PREPARED", "DISPATCHING", "SUBMITTED", "AMBIGUOUS"
    }
...
def build_protected_canary_preview(
    ...
    ready = bool(
        schema_checks.get("all_valid")
        and stop_valid
        and quantity_capped
        and explicit_arm_requested
        and journal_clear
        and flat_position
        and writer_preview.get("quantity_validation", {}).get("all_valid")
    )

    return {
        "stage": STAGE,
        "status": "READY_FOR_R36F12" if ready else "BLOCKED",
        "ready_for_r36f12": ready,
        "direction": direction,
        "entry_quantity": decimal_to_string(entry_quantity),
        "canary_max_entry_quantity": decimal_to_string(CANARY_MAX_ENTRY_QUANTITY),
        "quantity_capped": quantity_capped,
        "stop_price": decimal_to_string(stop_price),
        "stop_valid": stop_valid,
        "stop_working_type": CANARY_STOP_WORKING_TYPE,
        "explicit_arm_requested": bool(explicit_arm_requested),
        "journal_clear": journal_clear,
        "flat_position": bool(flat_position),
        "writer_schema_valid": bool(schema_checks.get("all_valid")),
        "writer_schema_checks": schema_checks,
        "protected_entry_request": protected_entry,
        "tp1_request": writer_preview["legs"]["tp1"],
        "tp2_request": writer_preview["legs"]["tp2"],
        "tp3_request": writer_preview["legs"]["tp3"],
        "submitted": False,
        "exchange_request_sent": False,
        "r36f12_transport_hard_disabled": True,
    }
...
def synthetic_r36f12_writer_safety_tests():
    ...
    synthetic_rows = [
        [0, "80000", "80200", "79900", "80100", "1"],
        [1, "80100", "80300", "80000", "80200", "1"],
        [2, "80200", "80400", "80100", "80300", "1"],
        [3, "80300", "80500", "80200", "80400", "1"],
        [4, "80400", "80600", "80300", "80500", "1"],
    ]
    # Use deterministic approved TP snapshot directly so this test targets
    # writer safety rather than historical clustering.
    tp_snapshot = {
        "tp_approval": {"approved": True, "status": "APPROVED", "reason": "SYNTHETIC"},
        "tp1": "80100",
        "tp2": "80300",
        "tp3": "TRAILING",
    }
    ...
    clear = build_protected_canary_preview(
        preview, Decimal("79600"), True, {}, True,
    )
...
async def run_r36f12():
    ...
    try:
        ...
        if EMA_SIGNAL_SNAPSHOT.get("ready"):
            ...
            log(
                f"R36F.12 EMA SNAPSHOT price={EMA_SIGNAL_SNAPSHOT.get('price')} "
            ...
            ideal_alert = build_ideal_condition_alert(
                EMA_SIGNAL_SNAPSHOT
            )
    ...
    try:
        ...
        if selected_direction is None:
            ...
            if not balance_readiness[
                ...
                check(
                    "R36F13_LIVE_TRANSPORT_REMAINS_DISABLED",
                    EXCHANGE_MUTATION_TRANSPORT_ENABLED
                    is False
                    and
                    ORDER_SUBMISSION_ENABLED
