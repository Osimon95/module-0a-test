# ============================================================================
# R28 UNIT N.26
# DURABLE RECOVERY CERTIFICATE + WAL + GENERATION FENCING
#
...
class SyntheticTransport:
    ...
    def post(
        ...
        if not SYNTHETIC_TRANSPORT_ONLY:
            raise RuntimeError(
                "synthetic transport requirement disabled"
            )
...
print(
    "R28 UNIT N.26: PART 1 DEFINITIONS LOADED",
    flush=True,
)
# ============================================================================
# R28 UNIT N.26
...
class N26Engine:
    ...
    def acquire_recovery_lease(
        ...
        with self._lock:
            ...
            lease = RecoveryLease(
                ...
                nonce=(
                    self.state.lease_nonce_counter
                ),
            )
    ...
    def issue_recovery_certificate(
        ...
        with self._lock:
            ...
            self.state.certificates.append(
                cert
            )
    ...
    def complete_generation(
        ...
        with self._lock:
            ...
            return (
                self.issue_recovery_certificate()
            )
...
def final_assessment() -> None:
    ...
    print(
        "Concurrent Single-Dispatch Safety = ✅ VERIFIED",
        flush=True,
    )
...
def main() -> None:
    print(
        f"{UNIT_NAME}: STARTING HEALTH SERVER",
        flush=True,
    )

    # IMPORTANT:
    # Start the Render health endpoint before running
    # the diagnostic suite. This prevents the service
    # from waiting for all tests before binding PORT.
    start_health_server()

    print(
        f"{UNIT_NAME}: HEALTH SERVER START REQUESTED",
        flush=True,
    )

    print(
        f"{UNIT_NAME}: STARTING TEST SUITE",
        flush=True,
    )
    ...
    print(
        f"{UNIT_NAME}: TEST SUITE COMPLETE",
        flush=True,
    )
...
print(
    "R28 UNIT N.26: PART 4 DEFINITIONS LOADED",
    flush=True,
)
