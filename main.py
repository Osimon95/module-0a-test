...
def run_tp_engine(rows, entry_price, direction):
    if direction == 'LONG':
        values = historical_highs(rows)
        extrema = build_extrema(values)
    elif direction == 'SHORT':
    ...
    clusters = cluster_extrema(extrema)
    valid, invalid = validate_clusters(clusters, entry_price, direction)
    approval = evaluate_tp_approval({'valid_cluster_count': len(valid), 'failure_reason': 'ONLY_ONE_VALID_CLUSTER' if len(valid) == 1 else 'INSUFFICIENT_VALID_CLUSTERS'})
    if not approval['approved']:
        return {'approved': False, 'approval': approval, 'valid_clusters': valid, 'invalid_clusters': invalid}
    prices = calculate_tp_prices(entry_price, valid, direction)
    return {'approved': True, 'approval': approval, 'valid_clusters': valid, 'invalid_clusters': invalid, 'prices': prices}

def build_cluster_tp_snapshot(entry_price, rows, side, fill_label):
    global LAST_TP_APPROVAL
    entry_price = D(entry_price)
    diagnostics = build_cluster_diagnostics(rows, entry_price, side)
    approval = evaluate_tp_approval(diagnostics)
    LAST_TP_APPROVAL = approval
    if not approval['approved']:
        log(f"{side} TP SET REJECTED: {approval['reason']}")
        raise RuntimeError(f"{side} historical TP set rejected: requires at least {REQUIRED_TP_CLUSTERS} valid clusters; found {approval['available_valid_clusters']}")
    clusters = valid_clusters(rows, entry_price, side)
    if len(clusters) < REQUIRED_TP_CLUSTERS:
        raise RuntimeError('TP approval inconsistency: diagnostics approved but independent cluster extraction found fewer than two valid clusters')
    prices = calculate_tp_prices(entry_price, clusters, side)
    snapshot = {'fill_label': fill_label, 'side': side, 'entry_price': decimal_to_string(entry_price), 'historical_diagnostics': diagnostics, 'tp_approval': approval, 'tp1': decimal_to_string(prices['tp1']), 'tp2': decimal_to_string(prices['tp2']), 'tp3': {'type': 'TRAILING', 'allocation_percent': decimal_to_string(TP3_ALLOCATION_PERCENT), 'trailing_distance_percent': decimal_to_string(TP3_TRAILING_DISTANCE_PERCENT)}, 'cluster1_average': decimal_to_string(prices['cluster1_average']), 'cluster2_average': decimal_to_string(prices['cluster2_average']), 'primary_tp_immutable': True}
    log(f'{side} TP SET APPROVED WITH {len(clusters)} VALID CLUSTERS')
    log(f"{side} TP1 = {snapshot['tp1']} (20% adjustable progress)")
    log(f"{side} TP2 = {snapshot['tp2']} (50% adjustable progress)")
    log(f'{side} TP3 = {TP3_ALLOCATION_PERCENT}% trailing runner')
    return snapshot

def synthetic_cluster_tests():
    long_rows = [[1, '99000', '100000', '99500', '99500', '1'], [2, '99500', '100100', '99600', '99800', '1'], [3, '99600', '100000', '99500', '99700', '1'], [4, '99500', '101000', '99900', '100100', '1'], [5, '99900', '100200', '99500', '100000', '1'], [6, '99500', '101500', '100000', '100500', '1'], [7, '100000', '101000', '99500', '100500', '1'], [8, '99500', '101400', '99900', '100800', '1']]
    short_rows = [[1, '81000', '81500', '80000', '81000', '1'], [2, '81000', '81500', '80100', '80800', '1'], [3, '80800', '81400', '80050', '80500', '1'], [4, '80500', '81300', '79900', '80300', '1'], [5, '80300', '81200', '80000', '80500', '1'], [6, '80500', '81400', '79800', '80400', '1'], [7, '80400', '81300', '80100', '80600', '1'], [8, '80600', '81500', '79950', '80800', '1']]
    long_diagnostics = build_cluster_diagnostics(long_rows, Decimal('99000'), 'LONG')
    long_approval = evaluate_tp_approval(long_diagnostics)
    check('SYNTHETIC_LONG_TWO_CLUSTER_APPROVAL', long_approval['approved'] is True)
    short_diagnostics = build_cluster_diagnostics(short_rows, Decimal('82000'), 'SHORT')
    short_approval = evaluate_tp_approval(short_diagnostics)
    check('SYNTHETIC_SHORT_TWO_CLUSTER_APPROVAL', short_approval['approved'] is True)
    return (long_approval, short_approval)

def synthetic_tp_rejection_test():
    rows = [[1, '99000', '100000', '99500', '99500', '1'], [2, '99500', '100100', '99600', '99800', '1'], [3, '99600', '100000', '99500', '99700', '1'], [4, '99500', '100100', '99800', '99900', '1']]
    entry = Decimal('99500')
    diagnostics = build_cluster_diagnostics(rows, entry, 'LONG')
    approval = evaluate_tp_approval(diagnostics)
    check('ONE_CLUSTER_TP_REJECTED', approval['approved'] is False)
    check('ONE_CLUSTER_APPROVAL_STATUS_REJECTED', approval['status'] == 'REJECTED')
    check('ONE_CLUSTER_DOES_NOT_APPROVE_TP_SET', approval['available_valid_clusters'] < REQUIRED_TP_CLUSTERS)
    return approval

def build_canary_preview():
    return {'stage': STAGE, 'symbol': SYMBOL, 'real_order_execution': REAL_ORDER_EXECUTION, 'demo_order_execution': DEMO_ORDER_EXECUTION, 'exchange_mutation_transport_enabled': EXCHANGE_MUTATION_TRANSPORT_ENABLED, 'order_submission_enabled': ORDER_SUBMISSION_ENABLED, 'first_real_order_allowed': FIRST_REAL_ORDER_ALLOWED, 'submitted': False, 'exchange_request_sent': False}
...
def writer_client_id(direction, leg):
    value = f'R36F8-{direction}-{leg}-0001'
    if len(value) > 36:
        raise ValueError('writer client id exceeds WEEX limit')
    return value
...
def allocation_exactly_representable(entry_quantity, tp1_percent, tp2_percent, tp3_percent):
    entry_quantity = quantize_down(entry_quantity, QUANTITY_STEP)
    percentages = (D(tp1_percent), D(tp2_percent), D(tp3_percent))
    if sum(percentages) != Decimal('100'):
        return False
    quantities = [entry_quantity * percent / Decimal('100') for percent in percentages]
    return bool(entry_quantity >= MIN_QUANTITY and all((q >= MIN_QUANTITY for q in quantities)) and all((quantize_down(q, QUANTITY_STEP) == q for q in quantities)) and (sum(quantities) == entry_quantity))

def select_tp_allocation(entry_quantity):
    entry_quantity = quantize_down(entry_quantity, QUANTITY_STEP)
    preferred = (TP1_ALLOCATION_PERCENT, TP2_ALLOCATION_PERCENT, TP3_ALLOCATION_PERCENT)
    adjusted = (ADJUSTED_TP1_ALLOCATION_PERCENT, ADJUSTED_TP2_ALLOCATION_PERCENT, ADJUSTED_TP3_ALLOCATION_PERCENT)
    if allocation_exactly_representable(entry_quantity, *preferred):
        return {'tp1_percent': preferred[0], 'tp2_percent': preferred[1], 'tp3_percent': preferred[2], 'adjusted': False, 'label': '20/20/60'}
    if allocation_exactly_representable(entry_quantity, *adjusted):
        return {'tp1_percent': adjusted[0], 'tp2_percent': adjusted[1], 'tp3_percent': adjusted[2], 'adjusted': True, 'label': '25/25/50'}
    return None

def writer_quantities(entry_quantity):
    entry_quantity = quantize_down(entry_quantity, QUANTITY_STEP)
    allocation = select_tp_allocation(entry_quantity)
    if allocation is None:
        return (entry_quantity, Decimal('0'), Decimal('0'), Decimal('0'))
    tp1 = entry_quantity * allocation['tp1_percent'] / Decimal('100')
    tp2 = entry_quantity * allocation['tp2_percent'] / Decimal('100')
    tp3 = entry_quantity * allocation['tp3_percent'] / Decimal('100')
    return (entry_quantity, tp1, tp2, tp3)

def validate_writer_quantities(entry_quantity, tp1, tp2, tp3):
    allocation = select_tp_allocation(entry_quantity)
    if allocation is None:
        return {'allocation_selected': False, 'all_valid': False}
    exact_tp1 = entry_quantity * allocation['tp1_percent'] / Decimal('100')
    exact_tp2 = entry_quantity * allocation['tp2_percent'] / Decimal('100')
    exact_tp3 = entry_quantity * allocation['tp3_percent'] / Decimal('100')
    checks = {'allocation_selected': True, 'entry_on_step': quantize_down(entry_quantity, QUANTITY_STEP) == entry_quantity, 'tp1_on_step': quantize_down(tp1, QUANTITY_STEP) == tp1, 'tp2_on_step': quantize_down(tp2, QUANTITY_STEP) == tp2, 'tp3_on_step': quantize_down(tp3, QUANTITY_STEP) == tp3, 'entry_minimum': entry_quantity >= MIN_QUANTITY, 'tp1_minimum': tp1 >= MIN_QUANTITY, 'tp2_minimum': tp2 >= MIN_QUANTITY, 'tp3_minimum': tp3 >= MIN_QUANTITY, 'allocation_sum_exact': tp1 + tp2 + tp3 == entry_quantity, 'tp1_selected_percent_exact': tp1 == exact_tp1, 'tp2_selected_percent_exact': tp2 == exact_tp2, 'tp3_selected_percent_exact': tp3 == exact_tp3, 'tp3_non_negative': tp3 >= Decimal('0')}
...
def minimum_adjustable_tp_entry_quantity():
    ...
    for _ in range(100000):
        quantity, tp1, tp2, tp3 = writer_quantities(candidate)
        checks = validate_writer_quantities(quantity, tp1, tp2, tp3)
        if checks.get('all_valid'):
            return quantity
        candidate += QUANTITY_STEP
    raise RuntimeError('Unable to find adjustable TP minimum entry quantity')
...
async def run_r36f12():
    ...
    log(f'{STAGE}: {PURPOSE}')
    line()
    check('REAL_ORDER_EXECUTION_DISABLED', REAL_ORDER_EXECUTION is False)
    check('DEMO_ORDER_EXECUTION_DISABLED', DEMO_ORDER_EXECUTION is False)
    check('EXCHANGE_MUTATION_TRANSPORT_DISABLED', EXCHANGE_MUTATION_TRANSPORT_ENABLED is False)
    check('ORDER_SUBMISSION_DISABLED', ORDER_SUBMISSION_ENABLED is False)
    check('LEVERAGE_MUTATION_DISABLED', LEVERAGE_MUTATION_ENABLED is False)
    check('MARGIN_MODE_MUTATION_DISABLED', MARGIN_MODE_MUTATION_ENABLED is False)
    check('POSITION_MUTATION_DISABLED', POSITION_MUTATION_ENABLED is False)
    check('FIRST_REAL_ORDER_DISABLED', FIRST_REAL_ORDER_ALLOWED is False)
    check('R36F14_DEMO_POST_TRANSPORT_DISABLED', R36F14_DEMO_POST_TRANSPORT_ENABLED is False)
    check('R36F14_DEMO_ORDER_SUBMISSION_DISABLED', R36F14_DEMO_ORDER_SUBMISSION_ENABLED is False)
    check('R36F14_FIRST_DEMO_ORDER_DISABLED', R36F14_FIRST_DEMO_ORDER_ALLOWED is False)
    check('R36F15_REAL_MONEY_FIREBREAK_INTACT', REAL_ORDER_EXECUTION is False and EXCHANGE_MUTATION_TRANSPORT_ENABLED is False and (ORDER_SUBMISSION_ENABLED is False) and (FIRST_REAL_ORDER_ALLOWED is False))
    check('R36F15_DEMO_ONLY_TRANSPORT_ENABLED', R36F15_DEMO_POST_TRANSPORT_ENABLED is True and R36F15_DEMO_ORDER_SUBMISSION_ENABLED is True and (R36F15_FIRST_DEMO_ORDER_ALLOWED is True))
    check('WEEX_API_KEY_PRESENT', bool(os.getenv('WEEX_API_KEY')))
    check('WEEX_API_SECRET_PRESENT', bool(os.getenv('WEEX_API_SECRET')))
    check('WEEX_API_PASSPHRASE_PRESENT', bool(os.getenv('WEEX_API_PASSPHRASE')))
    r36a_ids = set()
    r36a_ids.update(collect_ids_from_file(R36A_DEDUPE_FILE))
    r36a_ids.update(collect_ids_from_file(R36A_DECISION_FILE))
    R36A_EVIDENCE_OK = OLD_R36A_UPDATE_ID in r36a_ids
    check('R36A_DURABLE_EVIDENCE', R36A_EVIDENCE_OK, f'EXPECTED_UPDATE_ID={OLD_R36A_UPDATE_ID}')
    r36c_ids = set()
    r36c_ids.update(collect_ids_from_file(R36C_DEDUPE_FILE))
    r36c_ids.update(collect_ids_from_file(R36C_DECISION_FILE))
    R36C_EVIDENCE_OK = R36C_UPDATE_ID in r36c_ids
    check('R36C_DURABLE_EVIDENCE', R36C_EVIDENCE_OK, f'EXPECTED_UPDATE_ID={R36C_UPDATE_ID}')
    r36d_snapshot = read_json_file(R36D_SNAPSHOT_FILE, default={})
    R36D_EVIDENCE_OK = bool(r36d_snapshot)
    check('R36D_SNAPSHOT_EVIDENCE', R36D_EVIDENCE_OK, f'path={R36D_SNAPSHOT_FILE}')
    DURABLE_EVIDENCE_OK = R36A_EVIDENCE_OK and R36C_EVIDENCE_OK and R36D_EVIDENCE_OK
    try:
        await reconcile_weex()
        WEEX_READ_ONLY_OK = True
        try:
            r36f14_demo_account = await r36f14_read_demo_account()
            diagnostic_check('R36F14_DEMO_READ_ONLY_RECONCILIATION', r36f14_demo_account.get('all_reads_successful', False), 'asset=' + str(r36f14_demo_account.get('demo_asset')) + ' available=' + str(r36f14_demo_account.get('available_balance')) + ' open_positions=' + str(r36f14_demo_account.get('open_demo_position_count')) + ' history_count=' + str(r36f14_demo_account.get('history_count')))
        except Exception as exc:
            r36f14_demo_account = {'all_reads_successful': False, 'error': str(exc)}
            diagnostic_check('R36F14_DEMO_READ_ONLY_RECONCILIATION', False, str(exc))
        diagnostic_check('WEEX_READ_ONLY_RECONCILIATION', True)
    except Exception as exc:
        WEEX_READ_ONLY_OK = False
        diagnostic_check('WEEX_READ_ONLY_RECONCILIATION', False, str(exc))
    synthetic_long = None
    synthetic_short = None
    try:
        synthetic_long, synthetic_short = synthetic_cluster_tests()
        check('SYNTHETIC_TP_ENGINE', True)
    except Exception as exc:
        check('SYNTHETIC_TP_ENGINE', False, str(exc))
    try:
        rejection = synthetic_tp_rejection_test()
        check('TP_APPROVAL_REJECTION_FLOW', rejection['approved'] is False)
    except Exception as exc:
        check('TP_APPROVAL_REJECTION_FLOW', False, str(exc))
    try:
        synthetic_writer_quantity_tests()
        check('STRICT_TP_QUANTITY_FEASIBILITY_TESTS', True)
        synthetic_balance_readiness_tests()
        check('ADJUSTABLE_TP_BALANCE_READINESS_TESTS', True)
        ...
        check('R36F12_WRITER_SAFETY_TESTS', True)
    except Exception as exc:
        check('STRICT_TP_QUANTITY_FEASIBILITY_TESTS', False, str(exc))
        check('ADJUSTABLE_TP_BALANCE_READINESS_TESTS', False, str(exc))
    historical_rows = []
    try:
        historical_rows = await load_historical_klines()
        check('REAL_HISTORICAL_KLINES_LOADED', len(historical_rows) >= 3, f'rows={len(historical_rows)}')
        global EMA_SIGNAL_SNAPSHOT
        global TELEGRAM_COMMAND_PREVIEW
        EMA_SIGNAL_SNAPSHOT = build_ema_signal_snapshot(historical_rows)
        diagnostic_check('R36F12_EMA_ENGINE_READY', EMA_SIGNAL_SNAPSHOT.get('ready') is True, f"reason={EMA_SIGNAL_SNAPSHOT.get('reason')}")
        if EMA_SIGNAL_SNAPSHOT.get('ready'):
            log(f"R36F.12 EMA SNAPSHOT price={EMA_SIGNAL_SNAPSHOT.get('price')} EMA19={EMA_SIGNAL_SNAPSHOT.get('ema19')} EMA50={EMA_SIGNAL_SNAPSHOT.get('ema50')} EMA200={EMA_SIGNAL_SNAPSHOT.get('ema200')} structure={EMA_SIGNAL_SNAPSHOT.get('structure')} ideal_direction={EMA_SIGNAL_SNAPSHOT.get('ideal_direction')} fresh_crossover={EMA_SIGNAL_SNAPSHOT.get('fresh_crossover')}")
            diagnostic_check('R36F12_EMA_19_50_SEPARATION_QUALITY', EMA_SIGNAL_SNAPSHOT.get('quality_ok') is True, f"separation={EMA_SIGNAL_SNAPSHOT.get('ema19_50_separation_percent')}%")
            ideal_alert = build_ideal_condition_alert(EMA_SIGNAL_SNAPSHOT)
            if ideal_alert:
                log('R36F.12 TELEGRAM IDEAL-CONDITION ALERT PREVIEW:')
                for alert_line in ideal_alert.splitlines():
                    log('      ' + alert_line)
                telegram_alert_result = await send_r36f12_telegram_alert(ideal_alert)
                diagnostic_check('R36F12_TELEGRAM_IDEAL_ALERT_PATH', True, f'enabled={R36F12_TELEGRAM_ALERTS_ENABLED} result={telegram_alert_result}')
            else:
                diagnostic_check('R36F12_TELEGRAM_IDEAL_ALERT_PATH', True, 'No current ideal EMA direction; no alert sent')
    except Exception as exc:
        check('REAL_HISTORICAL_KLINES_LOADED', False, str(exc))
    real_long_snapshot = None
    REAL_LONG_MARKET_ELIGIBLE = False
    if historical_rows and MARK_PRICE is not None:
        try:
            real_long_snapshot = build_cluster_tp_snapshot(MARK_PRICE, historical_rows, 'LONG', 'REAL_LONG_PREVIEW')
            LONG_DIAGNOSTICS = real_long_snapshot['historical_diagnostics']
            REAL_LONG_MARKET_ELIGIBLE = bool(real_long_snapshot['tp_approval']['approved'])
            diagnostic_check('REAL_LONG_TP_MARKET_ELIGIBILITY', REAL_LONG_MARKET_ELIGIBLE, 'TP_APPROVAL=' + real_long_snapshot['tp_approval']['status'])
            log('REAL_LONG_TP_APPROVAL=' + real_long_snapshot['tp_approval']['status'])
        except Exception as exc:
            LONG_DIAGNOSTICS = build_cluster_diagnostics(historical_rows, MARK_PRICE, 'LONG')
            approval = evaluate_tp_approval(LONG_DIAGNOSTICS)
            REAL_LONG_MARKET_ELIGIBLE = False
            diagnostic_check('REAL_LONG_TP_MARKET_ELIGIBILITY', False, 'TP_APPROVAL=' + approval['status'] + ' reason=' + approval['reason'] + ' error=' + str(exc))
    real_short_snapshot = None
    REAL_SHORT_MARKET_ELIGIBLE = False
    if historical_rows and MARK_PRICE is not None:
        try:
            real_short_snapshot = build_cluster_tp_snapshot(MARK_PRICE, historical_rows, 'SHORT', 'REAL_SHORT_PREVIEW')
            SHORT_DIAGNOSTICS = real_short_snapshot['historical_diagnostics']
            REAL_SHORT_MARKET_ELIGIBLE = bool(real_short_snapshot['tp_approval']['approved'])
            diagnostic_check('REAL_SHORT_TP_MARKET_ELIGIBILITY', REAL_SHORT_MARKET_ELIGIBLE, 'TP_APPROVAL=' + real_short_snapshot['tp_approval']['status'])
            log('REAL_SHORT_TP_APPROVAL=' + real_short_snapshot['tp_approval']['status'])
        except Exception as exc:
            SHORT_DIAGNOSTICS = build_cluster_diagnostics(historical_rows, MARK_PRICE, 'SHORT')
            approval = evaluate_tp_approval(SHORT_DIAGNOSTICS)
            REAL_SHORT_MARKET_ELIGIBLE = False
            diagnostic_check('REAL_SHORT_TP_MARKET_ELIGIBILITY', False, 'TP_APPROVAL=' + approval['status'] + ' reason=' + approval['reason'] + ' error=' + str(exc))
    canary_preview = None
    try:
        current_command = os.getenv('R36F12_TELEGRAM_COMMAND_TEXT', '').strip()
        if current_command:
            TELEGRAM_COMMAND_PREVIEW = validate_telegram_command_against_signal(current_command, EMA_SIGNAL_SNAPSHOT, REAL_LONG_MARKET_ELIGIBLE, REAL_SHORT_MARKET_ELIGIBLE)
            log(f'R36F.12 TELEGRAM COMMAND PREVIEW = {TELEGRAM_COMMAND_PREVIEW}')
        else:
            TELEGRAM_COMMAND_PREVIEW = {'recognized': False, 'authorized_preview': False, 'reason': 'NO_COMMAND_SUPPLIED', 'exchange_order_sent': False}
        canary_preview = build_canary_preview()
        diagnostic_check('CANARY_PREVIEW', True)
    except Exception as exc:
        diagnostic_check('CANARY_PREVIEW', False, str(exc))
    writer_preview = None
    ...
    try:
        ...
        if REAL_SHORT_MARKET_ELIGIBLE and real_short_snapshot is not None:
            selected_direction = 'SHORT'
            selected_snapshot = real_short_snapshot
        elif REAL_LONG_MARKET_ELIGIBLE and real_long_snapshot is not None:
            selected_direction = 'LONG'
            selected_snapshot = real_long_snapshot
        if selected_direction is None:
            diagnostic_check('ADJUSTABLE_TP_BALANCE_READINESS', False, 'TRADE_NOT_ELIGIBLE: no currently eligible real market TP set')
            diagnostic_check('WRITER_REQUEST_CONSTRUCTION', False, 'blocked: no currently eligible real market TP set')
        elif AVAILABLE_BALANCE is None or MARK_PRICE is None:
            diagnostic_check('ADJUSTABLE_TP_BALANCE_READINESS', False, 'READINESS_UNAVAILABLE: balance or mark price unavailable')
            diagnostic_check('WRITER_REQUEST_CONSTRUCTION', False, 'blocked: balance or mark price unavailable')
        else:
            leverage = LEVERAGE_SHORT if selected_direction == 'SHORT' else LEVERAGE_LONG
            balance_readiness = evaluate_strict_tp_balance_readiness(AVAILABLE_BALANCE, MARK_PRICE, leverage)
            quantity_feasibility = evaluate_writer_quantity_feasibility(D(balance_readiness['planned_entry_quantity']))
            log('R36F.12 ADJUSTABLE TP BALANCE READINESS ' + 'direction=' + selected_direction + ' status=' + balance_readiness['status'] + ' reason=' + balance_readiness['reason'] + ' available_usdt=' + balance_readiness['available_balance'] + ' planned_entry_qty=' + balance_readiness['planned_entry_quantity'] + ' minimum_entry_qty=' + balance_readiness['minimum_strict_tp_entry_quantity'] + ' required_margin_usdt=' + balance_readiness['required_margin_for_minimum_qty'] + ' required_available_usdt=' + balance_readiness['required_available_balance'] + ' shortfall_usdt=' + balance_readiness['available_balance_shortfall'])
            diagnostic_check('ADJUSTABLE_TP_BALANCE_READINESS', balance_readiness['eligible'], 'status=' + balance_readiness['status'] + ' reason=' + balance_readiness['reason'])
            diagnostic_check('ADJUSTABLE_TP_QUANTITY_FEASIBILITY', quantity_feasibility['feasible'], 'reason=' + quantity_feasibility['reason'])
            if not balance_readiness['eligible']:
                log('R36F.11 TRADE_READINESS = REJECTED ' + 'reason=' + balance_readiness['reason'])
                diagnostic_check('WRITER_REQUEST_CONSTRUCTION', False, 'blocked before construction: ' + balance_readiness['reason'])
            else:
                quantity = D(balance_readiness['planned_entry_quantity'])
                log('R36F.11 TRADE_READINESS = ELIGIBLE ' + 'reason=' + balance_readiness['reason'])
                writer_preview = build_writer_request_preview(selected_direction, MARK_PRICE, quantity, selected_snapshot)
                WRITER_CONSTRUCTION_ELIGIBLE = bool(writer_preview['submitted'] is False and writer_preview['quantity_validation']['all_valid'] and (writer_preview['transport_enabled'] is False))
                diagnostic_check('WRITER_REQUEST_CONSTRUCTION', WRITER_CONSTRUCTION_ELIGIBLE, 'direction=' + selected_direction + ' quantity=' + writer_preview['entry_quantity'] + ' tp1_qty=' + writer_preview['tp1_quantity'] + ' tp2_qty=' + writer_preview['tp2_quantity'] + ' tp3_qty=' + writer_preview['tp3_quantity'])
                for validation_name, validation_result in writer_preview['quantity_validation'].items():
                    if validation_name == 'all_valid':
                        continue
                    diagnostic_check('WRITER_QUANTITY_' + validation_name.upper(), validation_result)
                canary_quantity = min(quantity, CANARY_MAX_ENTRY_QUANTITY)
                canary_writer_preview = build_writer_request_preview(selected_direction, MARK_PRICE, canary_quantity, selected_snapshot)
                production_journal = read_json_file(R36F12_CANARY_JOURNAL_FILE, default={})
                configured_stop = parse_canary_stop_price(CANARY_STOP_PRICE_TEXT)
                r36f13_stop_price = configured_stop if configured_stop is not None else calculate_r36f13_protective_stop(selected_direction, canary_writer_preview['entry_price'])
                r36f13_stop_checks = validate_r36f13_protective_stop(selected_direction, canary_writer_preview['entry_price'], r36f13_stop_price, canary_writer_preview['tp1'], canary_writer_preview['tp2'])
                diagnostic_check('R36F13_PROTECTIVE_STOP_CALCULATED_OR_CONFIGURED', r36f13_stop_checks['configured_or_calculated'], 'stop=' + decimal_to_string(r36f13_stop_price))
                for stop_check_name, stop_check_result in r36f13_stop_checks.items():
                    if stop_check_name in {'all_valid', 'configured_or_calculated'}:
                        continue
                    diagnostic_check('R36F13_PROTECTIVE_STOP_' + stop_check_name.upper(), stop_check_result)
                check('R36F13_PROTECTIVE_STOP_AUTHORIZATION_GATE', r36f13_stop_checks['all_valid'])
                r36f131_stop_envelope = validate_r36f131_stop_risk_envelope(selected_direction, canary_writer_preview['entry_price'], r36f13_stop_price, leverage)
                log('R36F.13.1 STOP RISK ENVELOPE ' + 'distance_percent=' + r36f131_stop_envelope['distance_percent'] + ' max_percent=' + r36f131_stop_envelope['configured_maximum_percent'] + ' leverage_reference_percent=' + r36f131_stop_envelope['leverage_reference_percent'])
                for envelope_check_name, envelope_check_result in r36f131_stop_envelope['checks'].items():
                    diagnostic_check('R36F131_STOP_RISK_ENVELOPE_' + envelope_check_name.upper(), envelope_check_result)
                check('R36F131_STOP_RISK_ENVELOPE_AUTHORIZATION_GATE', r36f131_stop_envelope['all_valid'])
                r36f132_stop_loss_budget = validate_r36f132_stop_loss_budget(canary_writer_preview['entry_price'], r36f13_stop_price, canary_writer_preview['entry_quantity'], AVAILABLE_BALANCE, leverage)
                log('R36F.13.2 STOP LOSS BUDGET ' + 'expected_loss_usdt=' + r36f132_stop_loss_budget['expected_loss_usdt'] + ' expected_loss_percent=' + r36f132_stop_loss_budget['expected_loss_percent_of_available_balance'] + ' max_account_loss_percent=' + r36f132_stop_loss_budget['configured_max_account_loss_percent'] + ' isolated_entry_margin_usdt=' + r36f132_stop_loss_budget['isolated_entry_margin_usdt'])
                for loss_check_name, loss_check_result in r36f132_stop_loss_budget['checks'].items():
                    if loss_check_name == 'all_valid':
                        continue
                    diagnostic_check('R36F132_STOP_LOSS_BUDGET_' + loss_check_name.upper(), loss_check_result)
                check('R36F132_STOP_LOSS_BUDGET_AUTHORIZATION_GATE', r36f132_stop_loss_budget['all_valid'])
                check('R36F152_SELECTED_TP_SNAPSHOT_AVAILABLE', selected_snapshot is not None)
                r36f14_demo_order_preview = build_r36f14_demo_order_preview(selected_direction, canary_writer_preview['entry_quantity'], selected_snapshot, r36f13_stop_price)
                r36f14_demo_order_validation = validate_r36f14_demo_order_preview(r36f14_demo_order_preview, selected_direction, canary_writer_preview['entry_price'])
                for demo_check_name, demo_check_result in r36f14_demo_order_validation['checks'].items():
                    if demo_check_name == 'all_valid':
                        continue
                    diagnostic_check('R36F14_DEMO_ORDER_' + demo_check_name.upper(), demo_check_result)
                check('R36F14_DEMO_ORDER_INTEGRATION_GATE', r36f14_demo_order_validation['all_valid'])
                protected_canary_preview = build_protected_canary_preview(canary_writer_preview, r36f13_stop_price, CANARY_ARM_REQUESTED, production_journal, len(OPEN_POSITIONS) == 0)
                diagnostic_check('R36F13_CANARY_QUANTITY_CAPPED', protected_canary_preview['quantity_capped'], 'max=0.0004 BTC')
                diagnostic_check('R36F13_DURABLE_JOURNAL_CLEAR', protected_canary_preview['journal_clear'])
                TELEGRAM_COMMAND_PREVIEW = apply_r36f13_stop_authorization_gate(TELEGRAM_COMMAND_PREVIEW, r36f13_stop_checks)
                TELEGRAM_COMMAND_PREVIEW = apply_r36f131_risk_envelope_authorization_gate(TELEGRAM_COMMAND_PREVIEW, r36f131_stop_envelope)
                TELEGRAM_COMMAND_PREVIEW = apply_r36f132_stop_loss_budget_authorization_gate(TELEGRAM_COMMAND_PREVIEW, r36f132_stop_loss_budget)
                log(f'R36F.15 DEMO ARM REQUESTED = {R36F15_DEMO_ARM_REQUESTED}')
                r36f15_demo_submission = await submit_r36f15_demo_order(r36f14_demo_order_preview, TELEGRAM_COMMAND_PREVIEW)
                log('R36F.15 DEMO SUBMISSION RESULT = ' + canonical_json(r36f15_demo_submission))
                if r36f15_demo_submission.get('attempted'):
                    check('R36F15_DEMO_ORDER_ACCEPTED', r36f15_demo_submission.get('accepted') is True, str(r36f15_demo_submission.get('transport')))
                    if r36f15_demo_submission.get('accepted'):
                        ...
                        diagnostic_check('R36F15_POST_TRADE_DEMO_RECONCILIATION', r36f15_demo_reconciliation_after.get('all_reads_successful') is True, canonical_json(r36f15_demo_reconciliation_after))
                else:
                    diagnostic_check('R36F15_DEMO_DISPATCH_NOT_ATTEMPTED', True, r36f15_demo_submission.get('reason'))
                check('R36F132_LIVE_TRANSPORT_REMAINS_DISABLED', EXCHANGE_MUTATION_TRANSPORT_ENABLED is False and ORDER_SUBMISSION_ENABLED is False and (REAL_ORDER_EXECUTION is False) and (FIRST_REAL_ORDER_ALLOWED is False))
    except Exception as exc:
        diagnostic_check('ADJUSTABLE_TP_BALANCE_READINESS', False, str(exc))
        diagnostic_check('WRITER_REQUEST_CONSTRUCTION', False, str(exc))
        check('R36F152_WRITER_DEMO_PIPELINE_EXCEPTION_FREE', False, str(exc))
    zero_write_conditions = REAL_ORDER_EXECUTION is False and DEMO_ORDER_EXECUTION is False and (EXCHANGE_MUTATION_TRANSPORT_ENABLED is False) and (ORDER_SUBMISSION_ENABLED is False) and (LEVERAGE_MUTATION_ENABLED is False) and (MARGIN_MODE_MUTATION_ENABLED is False) and (POSITION_MUTATION_ENABLED is False) and (FIRST_REAL_ORDER_ALLOWED is False) and (R36F14_DEMO_POST_TRANSPORT_ENABLED is False) and (R36F14_DEMO_ORDER_SUBMISSION_ENABLED is False) and (R36F14_FIRST_DEMO_ORDER_ALLOWED is False)
    ZERO_WRITE_INVARIANT_OK = zero_write_conditions
    check('REAL_MONEY_ZERO_WRITE_INVARIANTS', ZERO_WRITE_INVARIANT_OK)
    FINAL_GATE_OK = len(FINAL_BLOCKERS) == 0
    TEST_STATUS = 'PASS' if FINAL_GATE_OK else 'FAIL'
    line()
    log(f'{STAGE} FINAL STATUS = {TEST_STATUS}')
    log(f'{STAGE} FINAL_BLOCKER_COUNT = {len(FINAL_BLOCKERS)}')
    for blocker in FINAL_BLOCKERS:
        log(f'{STAGE} FINAL_BLOCKER = {blocker}')
    log(f'{STAGE} REAL_LONG_MARKET_ELIGIBLE = {REAL_LONG_MARKET_ELIGIBLE}')
    log(f'{STAGE} REAL_SHORT_MARKET_ELIGIBLE = {REAL_SHORT_MARKET_ELIGIBLE}')
    log(f'{STAGE} WRITER_CONSTRUCTION_ELIGIBLE = {WRITER_CONSTRUCTION_ELIGIBLE}')
    log(f"{STAGE} EMA_IDEAL_DIRECTION = {EMA_SIGNAL_SNAPSHOT.get('ideal_direction')}")
    log(f"{STAGE} EMA_STRUCTURE = {EMA_SIGNAL_SNAPSHOT.get('structure')}")
    log(f"{STAGE} TELEGRAM_COMMAND_AUTHORIZED_PREVIEW = {TELEGRAM_COMMAND_PREVIEW.get('authorized_preview', False)}")
    log(f'{STAGE} DEMO_ARM_REQUESTED = {R36F15_DEMO_ARM_REQUESTED}')
    log(f"{STAGE} DEMO_ORDER_ATTEMPTED = {bool(r36f15_demo_submission and r36f15_demo_submission.get('attempted'))}")
    log(f"{STAGE} DEMO_ORDER_ACCEPTED = {bool(r36f15_demo_submission and r36f15_demo_submission.get('accepted'))}")
    log(f'{STAGE} PROTECTIVE_STOP_PRICE = {(decimal_to_string(r36f13_stop_price) if r36f13_stop_price is not None else None)}')
    log(f"{STAGE} PROTECTIVE_STOP_VALID = {bool(r36f13_stop_checks and r36f13_stop_checks.get('all_valid'))}")
    log(f"{STAGE} PROTECTIVE_STOP_DISTANCE_PERCENT = {(r36f131_stop_envelope.get('distance_percent') if r36f131_stop_envelope else None)}")
    log(f"{STAGE} PROTECTIVE_STOP_RISK_ENVELOPE_VALID = {bool(r36f131_stop_envelope and r36f131_stop_envelope.get('all_valid'))}")
    log(f"{STAGE} PROTECTIVE_STOP_LOSS_BUDGET_VALID = {bool(r36f132_stop_loss_budget and r36f132_stop_loss_budget.get('all_valid'))}")
    ...
    log(f"{STAGE} STRICT_20_20_60_QUANTITY_FEASIBLE = {bool(quantity_feasibility and quantity_feasibility.get('feasible'))}")
    if quantity_feasibility:
        log(f"{STAGE} ADJUSTABLE_TP_MIN_ENTRY_QTY = {quantity_feasibility.get('minimum_required_entry_quantity')}")
    log(f"{STAGE} ADJUSTABLE_TP_BALANCE_READY = {bool(balance_readiness and balance_readiness.get('eligible'))}")
    if balance_readiness:
        log(f"{STAGE} TRADE_READINESS_STATUS = {balance_readiness.get('status')}")
        log(f"{STAGE} TRADE_READINESS_REASON = {balance_readiness.get('reason')}")
        log(f"{STAGE} REQUIRED_AVAILABLE_BALANCE = {balance_readiness.get('required_available_balance')}")
        log(f"{STAGE} AVAILABLE_BALANCE_SHORTFALL = {balance_readiness.get('available_balance_shortfall')}")
    snapshot = {'stage': STAGE, 'purpose': PURPOSE, 'timestamp': now_iso(), 'test_status': TEST_STATUS, 'final_gate_ok': FINAL_GATE_OK, 'final_blockers': FINAL_BLOCKERS, 'weex_read_only_ok': WEEX_READ_ONLY_OK, 'durable_evidence_ok': DURABLE_EVIDENCE_OK, 'r36a_evidence_ok': R36A_EVIDENCE_OK, 'r36c_evidence_ok': R36C_EVIDENCE_OK, 'r36d_evidence_ok': R36D_EVIDENCE_OK, 'zero_write_invariant_ok': ZERO_WRITE_INVARIANT_OK, 'mark_price': decimal_to_string(MARK_PRICE), 'available_balance': decimal_to_string(AVAILABLE_BALANCE), 'open_positions': OPEN_POSITIONS, 'long_diagnostics': LONG_DIAGNOSTICS, 'short_diagnostics': SHORT_DIAGNOSTICS, 'tp_policy': {'required_valid_clusters': REQUIRED_TP_CLUSTERS, 'tp1_progress_percent': decimal_to_string(TP1_PROFIT_MARGIN_PERCENT), 'tp2_progress_percent': decimal_to_string(TP2_PROFIT_MARGIN_PERCENT), 'tp3_allocation_percent': decimal_to_string(TP3_ALLOCATION_PERCENT), 'tp1_allocation_percent': decimal_to_string(TP1_ALLOCATION_PERCENT), 'tp2_allocation_percent': decimal_to_string(TP2_ALLOCATION_PERCENT), 'writer_quantity_policy': {'tp1_minimum': decimal_to_string(MIN_QUANTITY), 'tp2_minimum': decimal_to_string(MIN_QUANTITY), 'tp3_minimum': decimal_to_string(MIN_QUANTITY), 'strict_allocation': '20/20/60', 'minimum_strict_entry_quantity': decimal_to_string(minimum_strict_tp_entry_quantity()), 'minimum_leg_promotion_allowed': False, 'remainder_redistribution_allowed': False}, 'cluster_tolerance_percent': decimal_to_string(CLUSTER_TOLERANCE_PERCENT), 'minimum_cluster_touches': MIN_CLUSTER_TOUCHES}, 'synthetic_test_policy': {'long_requires_two_clusters': True, 'short_requires_two_clusters': True, 'one_cluster_must_reject': True, 'synthetic_fixtures_changed': True, 'production_tp_policy_changed': False}, 'market_eligibility': {'real_long_market_eligible': REAL_LONG_MARKET_ELIGIBLE, 'real_short_market_eligible': REAL_SHORT_MARKET_ELIGIBLE}, 'r36f12_ema_signal': EMA_SIGNAL_SNAPSHOT, 'r36f12_telegram_command_preview': TELEGRAM_COMMAND_PREVIEW, 'r36f12_telegram_policy': {'buy_command': TELEGRAM_BUY_COMMAND, 'sell_command': TELEGRAM_SELL_COMMAND, 'alerts_enabled': R36F12_TELEGRAM_ALERTS_ENABLED, 'command_can_execute_exchange_order': False, 'requires_ema_direction_agreement': True, 'requires_tp_market_eligibility': True}, 'canary_preview': canary_preview, 'writer_preview': writer_preview, 'r36f12_protected_canary_preview': protected_canary_preview, 'r36f13_protective_stop': {'distance_percent': decimal_to_string(R36F13_PROTECTIVE_STOP_DISTANCE_PERCENT), 'price': decimal_to_string(r36f13_stop_price) if r36f13_stop_price is not None else None, 'checks': r36f13_stop_checks, 'mandatory_before_authorization': True, 'exchange_order_sent': False}, 'r36f131_stop_risk_envelope': {'configured_maximum_percent': decimal_to_string(R36F131_MAX_PROTECTIVE_STOP_DISTANCE_PERCENT), 'evaluation': r36f131_stop_envelope, 'mandatory_before_authorization': True, 'leverage_reference_is_not_liquidation_price': True, 'exchange_order_sent': False}, 'r36f132_stop_loss_budget': {'configured_max_account_loss_percent': decimal_to_string(R36F132_MAX_ACCOUNT_LOSS_PERCENT), 'evaluation': r36f132_stop_loss_budget, 'mandatory_before_authorization': True, 'exchange_order_sent': False}, 'r36f12_canary_safety': {'max_entry_quantity': decimal_to_string(CANARY_MAX_ENTRY_QUANTITY), 'stop_price_configured': bool(CANARY_STOP_PRICE_TEXT), 'explicit_arm_requested': CANARY_ARM_REQUESTED, 'durable_journal_file': R36F12_CANARY_JOURNAL_FILE, 'durable_journal_unresolved': unresolved_canary_journal(read_json_file(R36F12_CANARY_JOURNAL_FILE, default={})), 'r36f12_zero_write': True}, 'strict_tp_quantity_feasibility': quantity_feasibility, 'strict_tp_balance_readiness': balance_readiness, 'trade_readiness': {'status': balance_readiness.get('status') if balance_readiness else 'UNAVAILABLE', 'reason': balance_readiness.get('reason') if balance_readiness else 'READINESS_NOT_EVALUATED', 'eligible': bool(balance_readiness and balance_readiness.get('eligible'))}, 'writer_construction_eligible': WRITER_CONSTRUCTION_ELIGIBLE, 'execution_firebreak': {'real_order_execution': REAL_ORDER_EXECUTION, 'demo_order_execution': DEMO_ORDER_EXECUTION, 'exchange_mutation_transport_enabled': EXCHANGE_MUTATION_TRANSPORT_ENABLED, 'order_submission_enabled': ORDER_SUBMISSION_ENABLED, 'leverage_mutation_enabled': LEVERAGE_MUTATION_ENABLED, 'margin_mode_mutation_enabled': MARGIN_MODE_MUTATION_ENABLED, 'position_mutation_enabled': POSITION_MUTATION_ENABLED, 'first_real_order_allowed': FIRST_REAL_ORDER_ALLOWED, 'demo_post_transport_enabled': R36F14_DEMO_POST_TRANSPORT_ENABLED, 'demo_order_submission_enabled': R36F14_DEMO_ORDER_SUBMISSION_ENABLED, 'first_demo_order_allowed': R36F14_FIRST_DEMO_ORDER_ALLOWED, 'r36f15_demo_post_transport_enabled': R36F15_DEMO_POST_TRANSPORT_ENABLED, 'r36f15_demo_order_submission_enabled': R36F15_DEMO_ORDER_SUBMISSION_ENABLED, 'r36f15_demo_arm_requested': R36F15_DEMO_ARM_REQUESTED}, 'r36f14_weex_demo_integration': {'demo_symbol': R36F14_DEMO_SYMBOL, 'demo_asset': R36F14_DEMO_ASSET, 'balance_endpoint': R36F14_DEMO_BALANCE_ENDPOINT, 'positions_endpoint': R36F14_DEMO_POSITIONS_ENDPOINT, 'history_endpoint': R36F14_DEMO_ORDER_HISTORY_ENDPOINT, 'order_endpoint': R36F14_DEMO_ORDER_ENDPOINT, 'read_only_reconciliation': r36f14_demo_account, 'synthetic_integration': r36f14_demo_integration, 'real_market_demo_order_preview': r36f14_demo_order_preview, 'real_market_demo_order_validation': r36f14_demo_order_validation, 'demo_post_transport_enabled': R36F14_DEMO_POST_TRANSPORT_ENABLED, 'demo_order_submission_enabled': R36F14_DEMO_ORDER_SUBMISSION_ENABLED, 'first_demo_order_allowed': R36F14_FIRST_DEMO_ORDER_ALLOWED, 'actual_demo_order_sent': bool(r36f15_demo_submission and r36f15_demo_submission.get('sent')), 'actual_demo_order_accepted': bool(r36f15_demo_submission and r36f15_demo_submission.get('accepted')), 'r36f15_demo_submission': r36f15_demo_submission, 'r36f15_post_trade_reconciliation': r36f15_demo_reconciliation_after, 'actual_real_order_sent': False}, 'writer_endpoints': {'entry': WRITER_ENDPOINT_ENTRY, 'tp1_tp2': WRITER_ENDPOINT_TPSL, 'tp3': WRITER_ENDPOINT_TRAILING}, 'writer_submission_policy': {'submitted': False, 'post_requests_sent': False, 'exchange_mutation_sent': False}}
    write_json_file(R36F_SNAPSHOT_FILE, snapshot)
    write_json_file(R36F12_TELEGRAM_SIGNAL_FILE, EMA_SIGNAL_SNAPSHOT)
    write_json_file(R36F12_TELEGRAM_COMMAND_FILE, TELEGRAM_COMMAND_PREVIEW)
    log(f'{STAGE} SNAPSHOT WRITTEN = {R36F_SNAPSHOT_FILE}')
    line()
    log('NO REAL ORDER WAS SENT')
    if r36f15_demo_submission and r36f15_demo_submission.get('sent'):
        log('WEEX DEMO ORDER WAS SENT; PRODUCTION REAL-MONEY ORDER REMAINS DISABLED')
    else:
        log('NO DEMO ORDER WAS SENT')
    log('NO PRODUCTION EXCHANGE MUTATION WAS SENT')
    line()
    return snapshot
...
async def heartbeat_loop():
    ...
    while True:
        HEARTBEAT_COUNT += 1
        log(f"HEARTBEAT stage={STAGE} status={TEST_STATUS} count={HEARTBEAT_COUNT} r36a_id={OLD_R36A_UPDATE_ID} tp1_margin={decimal_to_string(TP1_PROFIT_MARGIN_PERCENT)} tp2_margin={decimal_to_string(TP2_PROFIT_MARGIN_PERCENT)} required_clusters={REQUIRED_TP_CLUSTERS} long_valid_clusters={LONG_DIAGNOSTICS.get('valid_cluster_count')} short_valid_clusters={SHORT_DIAGNOSTICS.get('valid_cluster_count')} write_transport={EXCHANGE_MUTATION_TRANSPORT_ENABLED} real_execution={REAL_ORDER_EXECUTION} demo_arm={R36F15_DEMO_ARM_REQUESTED} reevaluation_seconds={R36F151_REEVALUATION_SECONDS}")
        await asyncio.sleep(R36F151_REEVALUATION_SECONDS)
        line()
        log(f'{STAGE} RUNTIME REEVALUATION START heartbeat={HEARTBEAT_COUNT}')
        line()
        try:
            await run_r36f12()
            log(f"{STAGE} RUNTIME REEVALUATION COMPLETE heartbeat={HEARTBEAT_COUNT} status={TEST_STATUS} long_valid_clusters={LONG_DIAGNOSTICS.get('valid_cluster_count')} short_valid_clusters={SHORT_DIAGNOSTICS.get('valid_cluster_count')}")
        except Exception as exc:
            TEST_STATUS = 'FAIL'
            line()
            log(f'{STAGE} RUNTIME REEVALUATION ERROR = {exc}')
            line()
...
async def async_main():
    ...
    try:
        ...
        TEST_STATUS = 'FAIL'
        line()
        log(f'{STAGE} UNHANDLED ERROR = {exc}')
        line()
