...
# ============================================================
#
# R24 IS STILL PRE-LIVE / DEMO ONLY.
#
# REAL ORDER TRANSMISSION MUST REMAIN DISABLED.
#
# R24 adds:
#
# 1. Order-state machine validation
# 2. Partial-fill delta accounting
# 3. Terminal-state locking
# 4. Duplicate exchange-event blocking
# 5. State-regression blocking
# 6. Actual demo-history idempotency validation
#
# ============================================================
...
async def blocked_real_post(*args, **kwargs):
    ...
    raise RuntimeError(
        "R24 ABSOLUTE SAFETY LOCK: "
        "real order POST is disabled"
    )
...
async def r24_run_diagnostic(
    ...
    # --------------------------------------------------------
    # R24 SYNTHETIC ORDER STATE TESTS
    # --------------------------------------------------------
    ...
    history_orig_qty = Decimal(
        "0"
    )

    history_exec_qty = Decimal(
        "0"
    )
    ...
    actual_tracker_fill_delta = Decimal(
        "0"
    )
    ...
    if RUN_DEMO_ORDER_TEST:
        ...
        # ====================================================
        # R24 ACTUAL EXCHANGE HISTORY IDEMPOTENCY
        # ====================================================
        #
        # Process actual exchange history once.
        #
        # Then deliberately feed the exact same event again.
        #
        # Second processing MUST be blocked.
        #
        # ====================================================
    ...
    lines = [
        ...
        f"Available USDT: "
        f"{dec_str(available_usdt)}",

        f"Mark Price: "
        f"{dec_str(mark_price)} USDT",
        ...
        f"{yesno(gate['fresh_signal'])}",

        f"Expired Signal Rejected: "
        f"{yesno(gate['expired_rejected'])}",

        f"Loss Cooldown Test: "
        f"{yesno(gate['loss_cooldown'])}",

        f"Duplicate Signal Rejected: "
        f"{yesno(gate['duplicate_rejected'])}",

        f"One Direction Gate: "
        f"{yesno(gate['one_direction_gate'])}",

        f"External Position Clear: "
        f"{yesno(gate['external_position_clear'])}",
        ...
        f"Entry: "
        f"{dec_str(ENTRY_PERCENT)}%",

        f"Leverage: "
        f"{dec_str(LEVERAGE)}x",

        f"Max Config Leverage: "
        f"{dec_str(MAX_CONFIG_LEVERAGE)}x",
        ...
        f"Pyramid Size: "
        f"{dec_str(PYRAMID_SIZE_PERCENT)}%",

        f"Max Backups: "
        f"{MAX_BACKUPS}",

        f"Backup Size: "
        f"{dec_str(BACKUP_SIZE_PERCENT)}% each",

        f"Backup Buffer: "
        f"{dec_str(BACKUP_BUFFER_PERCENT)}%",

        f"Min Liq Distance: "
        f"{dec_str(MIN_LIQ_DISTANCE_PERCENT)}%",
        ...
        f"Minimum Order: "
        f"{dec_str(contract.min_qty)}",

        f"Quantity Precision: "
        f"{contract.qty_precision}",

        f"Quantity Step: "
        f"{dec_str(contract.qty_step)}",

        f"Price Precision: "
        f"{contract.price_precision}",

        f"Price Step: "
        f"{dec_str(contract.price_step)}",

        f"Contract Value: "
        f"{dec_str(contract.contract_value)}",

        f"WEEX Min Leverage: "
        f"{dec_str(contract.min_leverage)}x",

        f"WEEX Max Leverage: "
        f"{dec_str(contract.max_leverage)}x",
        ...
        f"Margin: "
        f"{dec_str(margin)} USDT",

        f"Notional: "
        f"{dec_str(notional)} USDT",
        ...
        f"TP1 Trigger: "
        f"{dec_str(TP1_TRIGGER_PERCENT)}%",

        f"TP2 Trigger: "
        f"{dec_str(TP2_TRIGGER_PERCENT)}%",

        f"Trailing Distance: "
        f"{dec_str(TRAILING_DISTANCE_PERCENT)}%",
        ...
        f"{yesno(state_tests['new_accepted'])}",

        f"Partial Fill #1 Delta: "
        f"{yesno(state_tests['partial_1_delta'])}",

        f"Partial Fill #2 Delta: "
        f"{yesno(state_tests['partial_2_delta'])}",

        f"FILLED Terminal State: "
        f"{yesno(state_tests['filled_terminal'])}",
        ...
        f"Demo Time In Force: "
        f"{REHEARSAL_TIME_IN_FORCE}",

        f"Demo Limit Price: "
        f"{dec_str(demo_limit_price)}",

        f"Price Step Match: "
        f"{yesno(price_step_match)}",

        f"Demo POST Attempted: "
        f"{yesno(R24_DEMO_POST_ATTEMPTED)}",

        f"Demo POST Accepted: "
        f"{yesno(R24_DEMO_POST_ACCEPTED)}",

        f"Demo Order ID: "
        f"{demo_order_id}",

        f"History Lookup Attempted: "
        f"{yesno(history_lookup_attempted)}",
        ...
        f"History Order ID Match: "
        f"{yesno(history_order_id_match)}",

        f"History Symbol Match: "
        f"{yesno(history_symbol_match)}",

        f"History Side Match: "
        f"{yesno(history_side_match)}",

        f"History Position Side Match: "
        ...
        f"Status Recognized: "
        f"{yesno(status_recognized)}",

        f"Requested Quantity: "
        f"{dec_str(requested_qty)}",

        f"History Original Quantity: "
        f"{dec_str(history_orig_qty)}",

        f"History Executed Quantity: "
        f"{dec_str(history_exec_qty)}",

        f"Quantity Reconciliation: "
        f"{yesno(quantity_reconciliation)}",

        f"Lifecycle Validation: "
        f"{yesno(lifecycle_validation)}",
        ...
        f"First Processing Accepted: "
        f"{yesno(actual_tracker_first_accept)}",

        f"Duplicate Processing Blocked: "
        f"{yesno(actual_tracker_duplicate_blocked)}",

        f"Actual History Terminal: "
        f"{yesno(actual_tracker_terminal)}",

        f"Actual Fill Delta: "
        f"{dec_str(actual_tracker_fill_delta)}",
        ...
        f"{dec_str(position_before)}",

        f"Position Size After: "
        f"{dec_str(position_after)}",
...
async def main_async() -> None:
    ...
    #
    # Process then remains alive for Render.
    #
    # ========================================================
