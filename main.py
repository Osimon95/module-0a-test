#!/usr/bin/env python3
"""
R36F.2 - SMALLEST TP VALIDATION FIX

Purpose:
    Preserve the proven R36D/R36F.1 safety baseline while fixing ONLY
    the remaining REAL_SHORT_TP_PREVIEW_FAILED blocker.

R36F.2 CHANGE:
    - Keep the existing historical cluster TP engine.
    ...
    - For validation only, read additional consecutive historical 1m pages
      when the first 250 candles do not contain two valid clusters.
    - Maximum validation history = 1000 candles.
...
Frozen symbol convention:
    V3 account/private endpoints: BTCUSDT
    V2 ticker endpoint:           cmt_btcusdt
"""
...
PURPOSE = (
    "SMALLEST TP VALIDATION FIX: REAL LONG + SHORT HISTORICAL "
    "CLUSTER TP PREVIEWS WITH ADAPTIVE READ-ONLY HISTORY"
)
...
# R36F.2 validation-only extension.
# The first request remains exactly the frozen 250-candle request.
# Additional pages are requested only when necessary for validation.
...
# ======================================================================================
# R36F.2 HISTORICAL KLINE LOADING
# ======================================================================================
...
def load_historical_klines():
    ...
    """
    Read-only adaptive historical loader.

    Page 1:
        exactly the same 250 x 1m request
        used by R36F.1.

    Pages 2-4:
        requested only to extend validation history.
...
def cluster_extrema(values):
    ...
    """
    Frozen 0.20% price-tolerance clustering.

    The average is recalculated as additional
    qualifying extrema enter the same cluster.
    """
...
def run_r36f2():
    ...
    log(
        "R36F.2 Synthetic Cluster TP Test: "
        "LONG + SHORT"
    )
    ...
    check(
        "R36F.2 Performed Zero Exchange Writes",
        zero_write_ok,
    )
    ...
    check(
        "R36F.2 Final Pre-Live Production Gate",
        gate,
    )
