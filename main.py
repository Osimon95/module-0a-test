...
def writer_allocate_tp_quantities(
    ...
    if total_quantity < MIN_QUANTITY:
        raise ValueError(
            "writer total quantity below minimum"
        )
    ...
    tp1_percent = D(
        selected["tp1_percent"]
    )

    tp2_percent = D(
        selected["tp2_percent"]
    )

    tp3_percent = D(
        selected["tp3_percent"]
    )
    ...
    if (
        ...
        raise ValueError(
            "writer selected TP allocation produced "
            "a leg below MIN_QUANTITY"
        )
    ...
    if (
        ...
        raise ValueError(
            "writer TP quantities do not sum "
            "to total quantity"
        )
