...
#
# The real-order payload is built, signed, classified and
# rehearsed locally, but it is NEVER transmitted.
# ============================================================
...
# ============================================================
# R26.1 DIAGNOSTIC SETTINGS
# ============================================================

DEMO_ORDER_ENABLED = os.getenv(
    "DEMO_ORDER_ENABLED",
    "true",
).strip().lower() in {
    "1",
...
DEMO_PRICE_OFFSET_PERCENT = Decimal(
    os.getenv(
...
POSITION_POLL_DELAY_SECONDS = float(
    os.getenv(
        "POSITION_POLL_DELAY_SECONDS",
        "1.0",
    )
)
...
def parse_contract_info(
    ...
    try:
        qty_precision = (
            ...
            if qty_precision_raw
            is not None
            else precision_from_step(
                qty_step
            )
        )
...
# ============================================================
# R26.1 CLIENT ORDER ID / IDEMPOTENCY
# ============================================================
...
def rehearse_real_payload(
    ...
    cid1 = deterministic_client_order_id(
        intent,
        "r26",
    )

    cid2 = deterministic_client_order_id(
        intent,
        "r26",
    )
...
async def run_demo_lifecycle(
    ...
    if not DEMO_ORDER_ENABLED:
        raise RuntimeError(
            "DEMO_ORDER_ENABLED must remain "
            "true for R26.1 validation"
        )
...
def split_telegram_text(
    text: str,
    max_chars: int = TELEGRAM_CHUNK_SIZE,
) -> List[str]:
...
async def send_telegram(
    ...
    print(
        "TELEGRAM SEND STARTING: "
        f"{len(chunks)} message chunk(s)",
        flush=True,
    )
...
def build_report(
    ...
    lines = [
        ...
        (
            "API Trading Symbol: "
            f"{yesno(gate_results['api_symbol'])}"
        ),

        (
            "Fresh Signal Accepted: "
            f"{yesno(gate_results['fresh_signal'])}"
        ),

        (
            "Expired Signal Rejected: "
            f"{yesno(gate_results['expired_signal'])}"
        ),

        (
            "Loss Cooldown Test: "
            f"{yesno(gate_results['loss_cooldown'])}"
        ),

        (
            "Duplicate Signal Rejected: "
            f"{yesno(gate_results['duplicate_signal'])}"
        ),

        (
            "One Direction Gate: "
            f"{yesno(gate_results['one_direction'])}"
        ),

        (
            "External Position Clear: "
            f"{yesno(gate_results['external_position_clear'])}"
        ),
        ...
        (
            "Backup Size: "
            f"{decimal_text(BACKUP_SIZE_PERCENT)}% each"
        ),

        (
            "Backup Buffer: "
            f"{decimal_text(BACKUP_BUFFER_PERCENT)}%"
        ),

        (
            "Min Liq Distance: "
            f"{decimal_text(MIN_LIQ_DISTANCE_PERCENT)}%"
        ),
        ...
        (
            "Minimum Order: "
            f"{decimal_text(contract.min_qty)}"
        ),
        ...
        (
            "Quantity Step: "
            f"{decimal_text(contract.qty_step)}"
        ),
        ...
        (
            "Price Step: "
            f"{decimal_text(contract.price_step)}"
        ),

        (
            "Contract Value: "
            f"{decimal_text(contract.contract_value)}"
        ),
        ...
        (
            "Leverage Gate: "
            f"{yesno(contract.min_leverage <= LEVERAGE <= contract.max_leverage)}"
        ),
        ...
        (
            "Minimum Passed: "
            f"{yesno(quantity >= contract.min_qty)}"
        ),
        ...
        (
            "Initial: "
            f"{decimal_text(initial_exposure)}%"
        ),

        (
            "Pyramids: "
            f"{decimal_text(pyramid_exposure)}%"
        ),

        (
            "Backups: "
            f"{decimal_text(backup_exposure)}%"
        ),

        (
            "Total: "
            f"{decimal_text(total_exposure)}% / "
            f"{decimal_text(MAX_FUND_EXPOSURE_PERCENT)}%"
        ),

        (
            "Exposure Passed: "
            f"{yesno(total_exposure <= MAX_FUND_EXPOSURE_PERCENT)}"
        ),
        ...
        "R26.1 ORDER STATE MACHINE",

        (
            "NEW State Accepted: "
            f"{yesno(order_sm['new'])}"
        ),

        (
            "Partial Fill #1 Delta: "
            f"{yesno(order_sm['partial1'])}"
        ),

        (
            "Partial Fill #2 Delta: "
            f"{yesno(order_sm['partial2'])}"
        ),

        (
            "FILLED Terminal State: "
            f"{yesno(order_sm['filled'])}"
        ),

        (
            "Duplicate Exchange Event Blocked: "
            f"{yesno(order_sm['duplicate'])}"
        ),

        (
            "Terminal Regression Blocked: "
            f"{yesno(order_sm['terminal_regression'])}"
        ),

        "",

        "R26.1 EXECUTION INTENT GATE",

        (
            "Intent Created: "
            f"{yesno(intent_results['created'])}"
        ),

        (
            "Duplicate Intent Blocked: "
            f"{yesno(intent_results['duplicate'])}"
        ),

        (
            "NEW → PREFLIGHT: "
            f"{yesno(intent_results['to_preflight'])}"
        ),

        (
            "PREFLIGHT → READY: "
            f"{yesno(intent_results['to_ready'])}"
        ),

        (
            "Expired Intent Rejected: "
            f"{yesno(intent_results['expired'])}"
        ),

        (
            "Terminal Intent Regression Blocked: "
            f"{yesno(intent_results['terminal_regression'])}"
        ),

        "",

        "R26.1 EXECUTION PREFLIGHT",

        (
            "Live Execution OFF: "
            f"{yesno(preflight['live_off'])}"
        ),

        (
            "Hard Real POST Lock: "
            f"{yesno(preflight['hard_lock'])}"
        ),

        (
            "Intent Fresh: "
            f"{yesno(preflight['fresh'])}"
        ),

        (
            "Intent Quantity Positive: "
            f"{yesno(preflight['qty_positive'])}"
        ),

        (
            "Intent Minimum Passed: "
            f"{yesno(preflight['minimum'])}"
        ),

        (
            "Intent Leverage Passed: "
            f"{yesno(preflight['leverage'])}"
        ),

        (
            "Intent Exposure Passed: "
            f"{yesno(preflight['exposure'])}"
        ),

        (
            "Real Order Path Blocked: "
            f"{yesno(preflight['real_blocked'])}"
        ),

        (
            "Overall Preflight: "
            f"{yesno(overall_preflight)}"
        ),

        "",

        "R26.1 LIVE PAYLOAD REHEARSAL",
        ...
        (
            "Payload Built: "
            f"{yesno(bool(rehearsal.payload))}"
        ),

        (
            "Required Fields Present: "
            f"{yesno(rehearsal.required_fields_present)}"
        ),

        (
            "Client Order ID: "
            f"{rehearsal.payload.get('newClientOrderId', '')}"
        ),

        (
            "Client Order ID Valid: "
            f"{yesno(rehearsal.client_id_valid)}"
        ),

        (
            "Deterministic Client ID: "
            f"{yesno(rehearsal.deterministic_rebuild_match)}"
        ),

        (
            "Quantity Step Match: "
            f"{yesno(rehearsal.quantity_step_match)}"
        ),

        (
            "Price Step Match: "
            f"{yesno(rehearsal.price_step_match)}"
        ),

        (
            "Signature Generated Locally: "
            f"{yesno(rehearsal.signature_generated)}"
        ),

        (
            "Accepted Response Classifier: "
            f"{yesno(rehearsal.response_accept_classification_test)}"
        ),

        (
            "Rejected Response Classifier: "
            f"{yesno(rehearsal.response_reject_classification_test)}"
        ),

        (
            "Ambiguous Response Fails Closed: "
            f"{yesno(rehearsal.ambiguous_response_classification_test)}"
        ),

        (
            "Real POST Transmission Blocked: "
            f"{yesno(rehearsal.real_path_blocked)}"
        ),

        "",

        "R26.1 DEMO ORDER LIFECYCLE",

        (
            "Demo Symbol: "
            f"{lifecycle.demo_symbol}"
        ),
        ...
        (
            "Demo Limit Price: "
            f"{decimal_text(lifecycle.limit_price)}"
        ),

        (
            "Price Step Match: "
            f"{yesno(lifecycle.price_step_match)}"
        ),

        (
            "Demo Client Order ID: "
            f"{lifecycle.client_order_id}"
        ),

        (
            "Client Order ID Valid: "
            f"{yesno(lifecycle.client_order_id_valid)}"
        ),

        (
            "Demo POST Attempted: "
            f"{yesno(lifecycle.post_attempted)}"
        ),

        (
            "Demo POST Accepted: "
            f"{yesno(lifecycle.post_accepted)}"
        ),
        ...
        (
            "Response Client ID Match: "
            f"{yesno(lifecycle.response_client_id_match)}"
        ),

        (
            "History Lookup Attempted: "
            f"{yesno(lifecycle.history_lookup_attempted)}"
        ),
        ...
        (
            ...
            f"{yesno(lifecycle.history_found)}"
        ),

        (
            "History Order ID Match: "
            f"{yesno(lifecycle.history_order_id_match)}"
        ),

        (
            "History Client ID Match: "
            f"{yesno(lifecycle.history_client_id_match)}"
        ),

        (
            "History Symbol Match: "
            f"{yesno(lifecycle.history_symbol_match)}"
        ),

        (
            "History Side Match: "
            f"{yesno(lifecycle.history_side_match)}"
        ),

        (
            "History Position Side Match: "
            f"{yesno(lifecycle.history_position_side_match)}"
        ),

        (
            "Demo Final Status: "
            f"{lifecycle.final_status}"
        ),

        (
            "Status Recognized: "
            f"{yesno(lifecycle.status_recognized)}"
        ),

        (
            "Requested Quantity: "
            f"{decimal_text(lifecycle.requested_qty)}"
        ),

        (
            "History Original Quantity: "
            f"{decimal_text(lifecycle.original_qty)}"
        ),

        (
            "History Executed Quantity: "
            f"{decimal_text(lifecycle.executed_qty)}"
        ),

        (
            "Quantity Reconciliation: "
            f"{yesno(lifecycle.quantity_reconciled)}"
        ),

        (
            "Lifecycle Validation: "
            f"{yesno(lifecycle.lifecycle_valid)}"
        ),

        "",

        "R26.1 ACTUAL HISTORY IDEMPOTENCY",

        (
            "First Processing Accepted: "
            f"{yesno(history_idem['first'])}"
        ),

        (
            "Duplicate Processing Blocked: "
            f"{yesno(history_idem['duplicate'])}"
        ),

        (
            "Actual History Terminal: "
            f"{yesno(history_idem['terminal'])}"
        ),

        (
            "Actual Fill Delta: "
            f"{decimal_text(history_idem['delta'])}"
        ),

        "",

        "R26.1 DEMO POSITION RECONCILIATION",

        (
            "Position Size Before: "
            f"{decimal_text(position_before)}"
        ),

        (
            "Position Size After: "
            f"{decimal_text(position_after)}"
        ),

        (
            "Position Reconciled: "
            f"{yesno(position_reconciled)}"
        ),

        "",

        "R26.1 FAILURE-PATH MATRIX",

        (
            "Duplicate Submit Blocked: "
            f"{yesno(failure_tests.duplicate_submit_blocked)}"
        ),

        (
            "Stale Intent Blocked: "
            f"{yesno(failure_tests.stale_intent_blocked)}"
        ),

        (
            "Invalid Quantity Blocked: "
            f"{yesno(failure_tests.invalid_quantity_blocked)}"
        ),

        (
            "Invalid Price Blocked: "
            f"{yesno(failure_tests.invalid_price_blocked)}"
        ),

        (
            "Invalid Client ID Blocked: "
            f"{yesno(failure_tests.invalid_client_id_blocked)}"
        ),

        (
            "Terminal Regression Blocked: "
            f"{yesno(failure_tests.terminal_regression_blocked)}"
        ),

        (
            "Real POST Blocked Before Network: "
            f"{yesno(failure_tests.real_post_blocked_before_network)}"
        ),

        (
            "Ambiguous Response Fails Closed: "
            f"{yesno(failure_tests.ambiguous_response_fail_closed)}"
        ),
        ...
        "R26.1 SIGNAL → INTENT → EXECUTION CHAIN",

        (
            "Signal Direction: "
            f"{final_intent.direction}"
        ),
        ...
        (
            "Intent Quantity: "
            f"{decimal_text(final_intent.quantity)}"
        ),

        (
            "Client Order ID: "
            f"{final_intent.client_order_id}"
        ),

        (
            "Final Intent State: "
            f"{final_intent.state}"
        ),

        (
            "Intent Reconciled: "
            f"{yesno(final_intent.state == 'RECONCILED')}"
        ),

        "",

        "R26.1 RENDER PERSISTENCE",
        ...
        "🛡 R26.1 absolute real-order POST lock active",
    ...
    try:
        ...
        for position_attempt in range(
            ...
            if (
                ...
                print(
                    "DEMO POSITION RECONCILED: "
                    f"✅ attempt "
                    f"{position_attempt}/"
                    f"{POSITION_POLL_ATTEMPTS}",
                    flush=True,
                )
            ...
            print(
                "DEMO POSITION WAITING: "
                f"attempt "
                f"{position_attempt}/"
                f"{POSITION_POLL_ATTEMPTS} "
                f"expected="
                f"{decimal_text(expected_after)} "
                f"actual="
                f"{decimal_text(position_after)}",
                flush=True,
            )
        ...
        if not all(
            ...
            raise RuntimeError(
                "R26.1 failure matrix failed: "
                f"{failure_tests}"
            )

        stage = "intent finalization"

        # SUBMITTED represents DEMO submission only.
